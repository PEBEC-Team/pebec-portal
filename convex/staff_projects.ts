...
export const createProject = mutation({
  args: {
    ...
    steps: v.array(v.object({ title: v.string(), completed: v.boolean() })),
  },
  handler: async (ctx, { name, description, createdBy, steps }) => {
  ...
  export const addStep = mutation({
    ...
    handler: async (ctx, { projectId, title }) => {
      ...
      const newSteps = [...project.steps, { title, completed: false }];
      ...
      const newStatus =
        completedCount === newSteps.length
          ? "completed"
          : completedCount > 0
          ? "in_progress"
          : "open";
  ...
  export const toggleStep = mutation({
    ...
    handler: async (ctx, { projectId, index }) => {
      ...
      await ctx.db.patch(projectId, { steps, progress });
    },
  });

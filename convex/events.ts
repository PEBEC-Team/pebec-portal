...
import { PDFDocument, rgb } from 'pdf-lib';import QRCode from 'qrcode';
...
export const createEvent = mutation({
  args: {
    ...
    ticketLimit: v.optional(v.number()),    vipTicketLimit: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    ...
    const event = await ctx.db.insert("events", {
      ...
      ticketLimit: args.ticketLimit,      vipTicketLimit: args.vipTicketLimit,
...
export const rsvpEvent = mutation({
  args: {
    ...
    firstName: v.optional(v.string()),    lastName: v.optional(v.string()),    phone: v.optional(v.string()),
    qrCode: v.string(),
  ...
  handler: async (
    ...
    if (!event) throw new Error("❌ Event not found");    const registrations = await ctx.db
    ...
    const ticketNumber = `PEBEC-EV-${day}${month}${year}-${index}`;    const registrationId = await ctx.db.insert("event_registrations", {
      eventId,
    ...
    console.log(`✅ RSVP stored. Registration ID: ${registrationId} — Ticket: ${ticketNumber}`);    let userEmail = email ?? "";
    if (!userEmail && userId) {
...
for (const admin of admins) {  await ctx.db.insert("notifications", {
    userId: admin._id,
    eventId,
...
export const getUserTickets = query({
  args: { clerkUserId: v.string() },  handler: async (ctx, { clerkUserId }) => {    const user = await ctx.db
      .query("users")
      .withIndex("byClerkUserId", (q) => q.eq("clerkUserId", clerkUserId))
      .first();

    if (!user) throw new Error("User not found");    const tickets = await ctx.db
      .query("event_registrations")
...
export const getEventById = query({
    ...
    handler: async (ctx) => {
      ...
      return Promise.all(
        events.map(async (event) => {
          const createdBy = await ctx.db.get(event.createdBy);          const registrations = await ctx.db
            .query("event_registrations")
  ...
  export const getTicketPdf = query({
    ...
    handler: async (ctx, { eventId, userId }) => {
      if (!userId) return null;      const registration = await ctx.db
        .query("event_registrations")
        .withIndex("byEvent", (q) => q.eq("eventId", eventId))
        .filter((q) => q.eq(q.field("userId"), userId))        .first();
  ...
  export const getEventRegistration = query({
    ...
    handler: async (ctx, { ticketNumber }) => {
      ...
      console.log(`📷 Cover Image URL fetched: ${coverImageUrl}`);      const questions = await ctx.db
...
return {
  ...
  event: event
    ...
    : {
        ...
        eventDate: null,
        coverImageUrl: null,
        location: "Unknown Location",        host: "Unknown Host",      },
  questions: questions || [],
...
export const editEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const event = await ctx.db.get(eventId);
    if (!event) throw new Error("Event not found");    const changes: string[] = [];
    ...
    if (event.eventType !== updateData.eventType)
      changes.push(`Type: "${event.eventType}" → "${updateData.eventType}"`);    await ctx.db.patch(eventId, {
      ...updateData,
      updatedAt: Date.now(),
    });    const registrations = await ctx.db
      .query("event_registrations")

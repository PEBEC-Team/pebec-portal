...
export const getMediaByCategory = query({
  args: { categoryId: v.id("mediaCategories") },
  ...
  handler: async (ctx) => {
    ...
    return Promise.all(
      media.map(async (item) => {
        const coverImageUrl = item.pictureIds?.[0]
          ? await ctx.storage.getUrl(item.pictureIds[0])
          : null;
...
export const getMediaById = query({
  ...
  handler: async (ctx, { mediaId }) => {
    ...
    const pictureUrls = await Promise.all(
      media.pictureIds.map((id) => ctx.storage.getUrl(id))
    );

...
export const submitLetter = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.sentTo) {
      ...
      if (recipient && recipient.email) {        await ctx.scheduler.runAfter(0, api.email.sendEmail, {
          to: recipient.email,
  ...
  handler: async (ctx, { storageId }) => {    const url = await ctx.storage.getUrl(storageId);
    if (!url) return null;    const fileRecord = await ctx.db
...
export const getAllStates = query({
  handler: async (ctx) => {
    const stateUsers = await ctx.db.query("users")
      .filter(q => q.neq(q.field("state"), undefined))
      .collect();
...
export const updateLetterStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const letter = await ctx.db.get(args.letterId);    if (
      !letter ||
      (letter.sentTo !== user._id && letter.assignedTo !== user._id)
    ) {
...
export const getLettersReceivedByUser = query({
  handler: async (ctx) => {
    ...
    const letters = await ctx.db
      .query("letters")
      .filter((q) => q.eq(q.field("sentTo"), user._id))
      .collect();
...
export const getBusinessLetterStats = query({
  handler: async (ctx) => {
    ...
    const statusCounts = letters.reduce(
      ...
      { pending: 0, acknowledged: 0, in_progress: 0, resolved: 0 }
    );

    const userBreakdown: Record<string, { streams: Record<string, number> }> = {};
    ...
    for (const letter of letters) {
      ...
      letter.assignedToName.forEach((name) => {
        if (!userBreakdown[name]) {
          userBreakdown[name] = { streams: {} };
        }
        const stream = letter.assignedStream || "Unknown Stream";
        userBreakdown[name].streams[stream] =
          (userBreakdown[name].streams[stream] || 0) + 1;
      });
    }
...
export const assignLetterToAuditor = mutation({
  ...
  handler: async (ctx, { letterIds, auditorId }) => {

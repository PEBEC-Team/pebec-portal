...
export const assignUserToMDA = mutation({
  ...
  handler: async (ctx, { clerkUserId, mdaName, description, phoneNumber }) => {
    type MDAType = {
      _id: Id<"mdas">;
    ...
    if (user.mdaId) {
      ...
      if (oldMda && oldMda.assignedUsers) {
        const updatedUsers = oldMda.assignedUsers.filter(
    ...
    let mda = await ctx.db
      .query("mdas")
      .withIndex("byName", (q) => q.eq("name", mdaName))
      .unique() as MDAType | null;
    ...
    if (!mda) {
      ...
      const alreadyAssigned = mda.assignedUsers.some(
    ...
    await ctx.db.patch(user._id, {
      mdaId: mda._id,
      mdaName: mda.name,
      ...(user.role === "user" ? { role: "mda" } : {}),
    });

    console.log(`✅ ${user.email} assigned to MDA: ${mda.name}`);
    return { success: true, message: `✅ User assigned to ${mda.name}` };
  },
});
...
export const assignTicketToMDA = mutation({
  args: { ticketId: v.id("tickets"), mdaId: v.id("mdas") },
  handler: async (ctx, { ticketId, mdaId }) => {
    ...
    await ctx.db.patch(ticketId, { assignedMDA: mdaId });

    return { success: true, message: "Ticket assigned to MDA successfully" };
  },
});
...
export const updateUserInConvex = mutation({
  ...
  handler: async (ctx, { clerkUserId, role, mdaId, mdaName, phoneNumber }) => {
    ...
    await ctx.db.patch(user._id, { role, mdaId, mdaName, phoneNumber });
  },
});
...
export const updateUserRoleInConvex = mutation({
  ...
  handler: async (ctx, { clerkUserId, role, staffStream, state, permissions }) => {
    const user = await ctx.db
      .query("users")
      .withIndex("byClerkUserId", (q) => q.eq("clerkUserId", clerkUserId))
      .unique();
    ...
    if (
      [ "state_governor", "saber_agent", "deputies", "magistrates"].includes(role)
    ) {
...
export const removeUserFromMDA = mutation({
  ...
  handler: async (ctx, { clerkUserId }) => {
    ...
    const updatedUsers = mda.assignedUsers.filter(
      (id) => id.toString() !== user._id.toString()
    );

    await ctx.db.patch(user._id, { mdaId: undefined, mdaName: undefined });

    if (updatedUsers.length === 0) {
      await ctx.db.delete(mda._id);
    } else {
      await ctx.db.patch(mda._id, { assignedUsers: updatedUsers });
    }
  },
});
...
export const removeMDAFromUsers = mutation({
  ...
  handler: async (ctx, { mdaId }) => {
    const users = await ctx.db
      .query("users")
      .filter((q) => q.eq(q.field("mdaId"), mdaId))
      .collect();
...
export const deleteMDA = mutation({
  ...
  handler: async (ctx, { mdaId }) => {
    ...
    await ctx.scheduler.runAfter(0, api.users.removeMDAFromUsers, { mdaId });
...
export const getGrowthStats = query(async ({ db }) => {
  const now = new Date();
  ...
  const usersLastMonth = (await db
    .query("users")
    .filter((q) => q.gte(q.field("_creationTime"), lastMonthStart))
    .collect()).length;

  const usersThisMonth = (await db
    .query("users")
    .filter((q) => q.and(q.gte(q.field("_creationTime"), currentMonthStart), q.lte(q.field("_creationTime"), lastMonthEnd)))
    .collect()).length;
  ...
  const mdasLastMonth = (await db
    .query("mdas")
    .filter((q) => q.gte(q.field("_creationTime"), lastMonthStart))
    .collect()).length;

  const mdasThisMonth = (await db
    .query("mdas")
    .filter((q) => q.and(q.gte(q.field("_creationTime"), currentMonthStart), q.lte(q.field("_creationTime"), lastMonthEnd)))
    .collect()).length;
  ...
  return {
    ...
    growth: {
      ...
      mdas: { current: mdasThisMonth, previous: mdasLastMonth },
    },
  };
});
...
export const getPendingRoleRequests = query({
  ...
  handler: async (ctx) => {
    ...
    return await ctx.db
      .query("users")
      .filter((q) => q.eq(q.field("roleRequest.status"), "pending"))
      .collect();
  },
});
...
export const generateMonthlyAccessCode = mutation({
  ...
  handler: async (ctx) => {
    ...
    const existing = await ctx.db
      .query("access_codes")
      .withIndex("byDate")
      .filter((q) => q.gte("generatedAt", firstOfMonth as any))
      .first();

...
export const generateUploadUrl = mutation({
  ...
  export const getAllReports = query({
    ...
    handler: async (ctx) => {
      ...
      return await Promise.all(reports.map(async (report) => {
        const coverUrl = report.reportCoverUrl
          ? await ctx.storage.getUrl(report.reportCoverUrl)
          : undefined;
  ...
  export const getReportsByDateRange = query({
    ...
    handler: async (ctx, { startDate, endDate }) => {
      return await ctx.db
        .query("reports")
        .filter((q) => q.gte(q.field("publishedAt"), startDate))
        .filter((q) => q.lte(q.field("publishedAt"), endDate))
        .collect();
    },
  });
  ...
  export const getUserReportsPerMonth = query({
    ...
    handler: async (ctx, { userId }) => {
      ...
      const reportCounts = reports.reduce((acc, report) => {
        const month = new Date(report.publishedAt).toLocaleString("default", { month: "short" });
...
export const getUserLettersPerMonth = query({
  ...
  handler: async (ctx, { userId }) => {
    ...
    const lettersPerMonth: { [key: string]: number } = {};
    letters.forEach((letter) => {
      const month = new Date(letter.letterDate).toLocaleString("default", { month: "long" });

...
export const getAllBusinessLetters = query({
  ...
  export const assignLettersToStaff = mutation({
    ...
    handler: async (ctx, { letterIds, staffIds, stream, staffNames }) => {
  ...
  export const updateLetterStatus = mutation({
    ...
    handler: async (ctx, { letterId, status }) => {
      ...
      await ctx.db.patch(letterId, { status });
  

      const staffUsers = await ctx.db
      .query("users")
      .filter((q) =>
        q.or(...(letter.assignedTo ?? []).map((id) => q.eq(q.field("_id"), id)))
      )
      .collect();

...
export default function NewPostForm({ initialData }: { initialData?: any }) {
  ...
  const { register, setValue, handleSubmit, formState: { errors, isSubmitting }, watch } = useForm<Inputs>({
    resolver: zodResolver(newPostSchema),
    defaultValues: {
      title: initialData?.title || "",
      slug: initialData?.slug || "",
      excerpt: initialData?.excerpt || "",
      content: initialData?.content ? JSON.parse(initialData.content) : { content: [] },
  ...
  const postQuery = slug ? useQuery(api.posts.getPostBySlug, { slug }) : null;
  ...
  ? { ...initialData, content: JSON.stringify(watchedContent) } 

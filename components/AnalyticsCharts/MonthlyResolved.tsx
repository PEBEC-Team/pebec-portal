...
export default function AdminTicketReport() {
  ...
  const leastMDAs = topMDAsSorted
    .filter((mda) => mda.mdaName !== "Unknown MDA" && !topMdaNames.includes(mda.mdaName))
    .slice(-topMdaCount);
  ...
  const exportPDF = (filename, tableHead, tableBody) => {
    ...
    autoTable(doc, { startY: 40, head: tableHead, body: tableBody });
    doc.save(filename);
  };

  const exportExcel = (filename, data) => {
    const ws = XLSX.utils.json_to_sheet([{ Period: periodText }, {}, ...data]);
    ...
    <div className="bg-white p-4 sm:p-6 rounded-xl shadow space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            onClick={() =>
              exportPDF("Overview_Report.pdf", [["Total", "Resolved Within 72h", "Closed Within 72h"]],
                [[stats?.total ?? 0, stats?.resolvedWithin72h ?? 0, stats?.closedWithin72h ?? 0]]
              )
            }
          ...
          <Button
            onClick={() =>
              exportExcel("Overview_Report.xlsx", [{
                Total: stats?.total ?? 0,
  ...
  />
        ...
        {/* Least Performing MDA Chart */}
        {canSeeLeastMDA && (
          <div className="border p-4 rounded-lg h-[300px]">
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
            ...
            {leastMDAs.length > 0 ? (
              ...
              <p className="text-sm text-gray-500 italic">No least performing MDAs to display.</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className={`pt-4 border-t mt-6 flex flex-wrap gap-2 ${!canSeeLeastMDA ? "justify-center" : "justify-start"}`}>
        <Button
          onClick={() => exportPDF("Top_MDA_Report.pdf", [["Rank", "MDA", "Resolved+Closed"]],
            topMdaList.map((mda, i) => [i + 1, mda.mdaName, mda.count])
          )}
          className="bg-blue-600 text-white"
        >
        ...
        <Button
          onClick={() =>
            exportExcel("Top_MDA_Report.xlsx", topMdaList.map((mda, i) => ({
              Rank: i + 1,
              MDA: mda.mdaName,

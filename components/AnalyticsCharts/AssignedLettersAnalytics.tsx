...
export default function LetterStatsDashboard() {
  ...
  const data = [
    { name: STATUS_LABELS.pending, value: stats.pending },
    { name: STATUS_LABELS.acknowledged, value: stats.acknowledged },
    { name: STATUS_LABELS.in_progress, value: stats.in_progress },
    { name: STATUS_LABELS.resolved, value: stats.resolved },
  ];
      ...
      <Card className="flex flex-col justify-between">
        ...
        <CardContent>
          ...
          <div className="mt-4 space-y-3">
            {Object.entries(userBreakdown as Record<string, { streams: Record<string, number> }>).map(
              ([userName, info]) => (
                <div key={userName}>
                  <h4 className="font-medium capitalize">{userName}</h4>
                  <ul className="ml-4 list-disc text-sm text-gray-600">
                    {Object.entries(info.streams).map(([stream, count]) => (
                      <li key={stream}>

...
export default function MdaChart() {
  ...
  const barData = Object.entries(mdaData)
    .filter(([_, stats]) => stats.mdaName && stats.mdaName !== "Unknown MDA")
    .map(([_, stats]) => ({
  ...
  const chartData: { [key: string]: string | number }[] = isOverview
    ? barData.map(({ mdaName, total }) => ({ label: mdaName, count: total }))
    : (() => {
        const selectedStats = barData.find((entry) => entry.mdaName === selectedMDA);
        return selectedStats
          ? [
              { label: "Open", count: selectedStats.Open },
              { label: "In Progress", count: selectedStats["In Progress"] },
              { label: "Resolved", count: selectedStats.Resolved },
              { label: "Closed", count: selectedStats.Closed },
            ]
          : [];
      })();
  ...
  return (
    <div className="bg-[#0F172A] text-white p-6 rounded-2xl shadow-xl w-full overflow-x-auto">
      {/* Header */}
  ...
  className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 max-w-[200px] truncate"
  title={selectedMDA}
>
  {selectedMDA}

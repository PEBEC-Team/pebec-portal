...
export default function StatsCards() {
  const stats = useQuery(api.users.getGrowthStats) || {
    total: { users: 0, posts: 0, mdas: 0 },
    growth: {
      users: { current: 0, previous: 0 },
      posts: { current: 0, previous: 0 },
      mdas: { current: 0, previous: 0 },
    },
  };
  ...
  const resolvedPercentage =
    totalActiveTickets > 0
      ? Math.round((ticketStats.resolved / totalActiveTickets) * 100)
      : 0;
  ...
  const userGrowth = getPercentageChange(
    stats.growth.users.current,
    stats.growth.users.previous
  );
  const postGrowth = getPercentageChange(
    stats.growth.posts.current,
    stats.growth.posts.previous
  );
  const mdaGrowth = getPercentageChange(
    stats.growth.mdas.current,
    stats.growth.mdas.previous
  );

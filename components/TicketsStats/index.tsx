...
export default function TicketSummary() {
  ...
  const handleExport = () => {
    ...
    const data = [
      { Metric: "MDA", Value: mdas?.find((m) => m._id === mdaId)?.name ?? "All" },
      { Metric: "From", Value: from?.toLocaleDateString() ?? "N/A" },
      { Metric: "To", Value: to?.toLocaleDateString() ?? "N/A" },
      {},
      ...
      { Metric: "Resolved ≤ 72h", Value: stats.resolvedWithin72h },
      { Metric: "Closed ≤ 72h", Value: stats.closedWithin72h },
      { Metric: "Avg Resolution Time (hrs)", Value: stats.avgResolutionTime },
      { Metric: "Avg First Response Time (hrs)", Value: stats.avgResponseTime },
    ];
    ...
    datasets: [
      {
        ...
        data: stats ? [
          ...
          stats.open,
          stats.overdue,
          stats.resolvedWithin72h,
          stats.closedWithin72h,
        ] : [],
  ...
  const sparklineMock = (base: number) =>
    Array.from({ length: 6 }, (_, i) => base * (0.8 + Math.random() * 0.4));
...
<div className="bg-white rounded-xl p-4 shadow-sm border flex flex-wrap items-end justify-between gap-4">
  <div className="flex flex-wrap gap-4 items-end">
    ...
    <div className="w-60">
      ...
      <Select
        value={mdas.map((m) => ({ label: m.name, value: m._id })).find((opt) => opt.value === mdaId) || null}
        onChange={(opt) => setMdaId(opt?.value || "")}
        options={mdas.map((m) => ({ label: m.name, value: m._id }))}
  ...
  <div className="flex gap-2 flex-wrap">
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[{
          label: "Total Tickets", value: stats.totalTickets, icon: Briefcase, color: "bg-blue-100 text-blue-800", spark: sparklineMock(stats.totalTickets),
        }, {
          label: "Resolved", value: stats.resolved, icon: CheckCircle, color: "bg-green-100 text-green-800", spark: sparklineMock(stats.resolved),
        }, {
          label: "Open", value: stats.open, icon: Clock, color: "bg-yellow-100 text-yellow-800", spark: sparklineMock(stats.open),
        }, {
          label: "Overdue", value: stats.overdue, icon: AlertTriangle, color: "bg-red-100 text-red-800", spark: sparklineMock(stats.overdue),
          ...
          <div key={label} className={`rounded-xl p-4 border shadow-sm ${color}`}>
            ...
            <Sparklines data={spark} width={100} height={30}>
              <SparklinesLine style={{ stroke: "currentColor", fill: "none" }} />
            </Sparklines>
          </div>
        ))}
      </div>
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((card, index) => (
          <div key={index} className="bg-white p-5 rounded-xl shadow border flex flex-col justify-between">

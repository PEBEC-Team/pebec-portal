...
interface Props {
  setFileIds: (ids: { storageId: string; fileName: string }[]) => void;
}

export default function MultiFileUploader({ setFileIds }: Props) {
  const [files, setFiles] = useState<File[]>([]);
  const [uploading, setUploading] = useState(false);
  const [uploadedFiles, setUploadedFiles] = useState<{ storageId: string; fileName: string }[]>([]);

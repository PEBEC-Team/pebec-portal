...
export default function InternalRoleRequestForm({ user }: { user: any }) {
  ...
  const rolesList: { value: AllowedRoles; label: string }[] = [
    { value: "mda", label: "Ministries, Departments & Agencies (MDA)" },
    { value: "reform_champion", label: "Reform Champion" },
    { value: "saber_agent", label: "Saber" },
    { value: "deputies", label: "Deputy" },
    { value: "magistrates", label: "Magistrate" },
    { value: "state_governor", label: "State Governor" },
  ];
  ...
  if (user?.roleRequest?.status === "pending") {
    return (
      <p className="text-yellow-500 font-semibold text-center py-10">
  ...
  const handleSubmit = async () => {
    if (
      ...
      !form.phoneNumber ||
      !form.requestedRole ||
      (["mda", "reform_champion"].includes(form.requestedRole) && !form.mdaId)
 ||
      !form.state
    ) {
    ...
    try {
      ...
      // ✅ Defer the reset until after render
      setTimeout(() => {
        setForm({
          requestedRole: "mda",
          mdaId: "",
...
return (
    <div className="flex flex-col md:flex-row  bg-[#0A0E27] text-gray-900 px-4 py-10 items-center justify-center gap-10">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center">
        ...
        {submitted && (
  <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-6 text-sm flex items-center gap-2">
    ...
    Request submitted successfully! An admin will look into it and you'll be notified once your account has been approved.
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
              ...
              <Input className="pl-10" placeholder="First Name" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} />
            </div>
            <div className="relative">
              <UserIcon className="w-5 h-5 absolute left-3 top-3 text-gray-400" />
              <Input className="pl-10" placeholder="Last Name" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} />
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <Input className="pl-10" placeholder="Job Title" value={form.jobTitle} onChange={(e) => setForm({ ...form, jobTitle: e.target.value })} />
            </div>
  
            <div className="relative">
              <MapPinIcon className="w-5 h-5 absolute left-3 top-3 text-gray-400" />
              <Input className="pl-10" placeholder="Address" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
            </div>
  
            <div className="relative">
              <PhoneIcon className="w-5 h-5 absolute left-3 top-3 text-gray-400" />
              <Input className="pl-10" placeholder="Phone Number" value={form.phoneNumber} onChange={(e) => setForm({ ...form, phoneNumber: e.target.value })} />
            </div>
  
            <div>
              <Label className="text-sm mb-1 block text-gray-700">Role</Label>
              <Select value={form.requestedRole} onValueChange={(value) => setForm({ ...form, requestedRole: value as AllowedRoles })}>
                <SelectTrigger>
                  <SelectValue placeholder="Select Role" />
                </SelectTrigger>
                <SelectContent>
                {[
  { value: "mda", label: "Ministries, Departments & Agencies (MDA) - ReportGov Agent" },
  { value: "reform_champion", label: "Reform Champion" },
  { value: "saber_agent", label: "Saber" },
  { value: "deputies", label: "Deputy" },
  { value: "magistrates", label: "Magistrate" },
  ...
  <SelectItem key={role.value} value={role.value}>
    {role.label}
            ...
            {["mda", "reform_champion"].includes(form.requestedRole) && (
              <div>
                <Label className="text-sm mb-1 block text-gray-700">Select MDA</Label>
                <Select value={form.mdaId} onValueChange={(value) => setForm({ ...form, mdaId: value })}>
                  <SelectTrigger>

...
export default function TicketComments({ ticketId }: { ticketId: string }) {
  ...
  const comments = useQuery(
    api.ticket_comments.getTicketComments,
  ...
  const [attachedFiles, setAttachedFiles] = useState<{ id: Id<"_storage">; name: string }[]>([]);
  ...
  const handleFileAttach = (storageId: string, fileName: string) => {
    setAttachedFiles((prev) => [...prev, { id: storageId as Id<"_storage">, name: fileName }]);
  };
      ...
      await Promise.all(
        comments?.map(async (comment) => {
          ...
          const urls = await Promise.all(
            comment.fileIds.map(async (fileId) => {
              const url = await getStorageUrl({ storageId: fileId });
              return url || "";
            })
          );
  ...
  <div className="relative">
    <Textarea
      ...
      rows={3}
      className="resize-none border p-2 rounded-md pr-10"
    />
  ...
  <div className="mt-2 text-sm text-gray-700 border rounded-md p-3 bg-gray-50">
    ...
    <ul className="space-y-1">
      {attachedFiles.map((file, idx) => (
        <li key={idx} className="flex items-center justify-between text-sm text-gray-800 bg-white px-3 py-1 rounded border">
  ...
  Post
      ...
      <ul className="space-y-6">
        {comments?.map((comment) => {
          ...
          return (
            <li key={comment._id} className="pb-4 border-b last:border-b-0">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div>
                        ...
                        {comment.author?.jobTitle && (
                          <p className="text-xs text-gray-500 mt-0.5">{formatDisplay(comment.author.jobTitle)}</p>
                        )}
                      </div>
                    ...
                    {/* {isOwner && (
                      <Popover>
                        ...
                        <PopoverContent className="w-28 p-2 flex flex-col text-sm bg-white shadow-md border rounded-lg">
                          <button
                            onClick={() => {
                  ...
                  {fileUrls.length > 0 && (
                    <div className="mt-2 text-sm text-blue-600 space-y-1">
                      {fileUrls.map((url, idx) => (
                        <a key={idx} href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:underline">

...
export default function MdaTicketsDashboard() {
  ...
  const tickets = useQuery(api.tickets.getTicketsByMda, mdaId ? { mdaId } : "skip");
  ...
  const filteredTickets = tickets?.filter((ticket) => {
    ...
    ticket.createdByUser?.firstName?.toLowerCase().includes(searchLower) ||
    ticket.createdByUser?.lastName?.toLowerCase().includes(searchLower) ||
    (ticket.createdByUser as any)?.businessName?.toLowerCase().includes(searchLower);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8">
      ...
      {/* Filters + Ticket Report Generator Card */}
      <div className="flex flex-wrap items-start gap-4 mb-6 p-6 bg-white border rounded-lg shadow-lg">
        ...
        <div className="relative flex-1 min-w-[250px]">
          ...
          <input
            ...
            className="pl-10 p-3 border rounded-lg w-full h-[44px] focus:ring-2 focus:ring-blue-500 transition-all"
            value={searchQuery}
        ...
        <div className="relative min-w-[120px]">
          <select
            className="p-3 border rounded-lg w-full h-[44px] focus:ring-2 focus:ring-blue-500"
            value={statusFilter}
        ...
        <div className="relative flex items-center gap-2 min-w-[180px]">
          ...
          <div className="relative flex-1">
            ...
            <input
  type="date"
  className="pl-10 p-3 border rounded-lg w-full h-[44px] focus:ring-2 focus:ring-blue-500"
  ...
  >
    {ticket.status.replace("_", " ").toUpperCase()}
      ...
      {showTicketReportModal && (
        <GenerateTicketReport open={showTicketReportModal} onClose={() => setShowTicketReportModal(false)} />
      )}
      
      {showMonthlyReportModal && (
  <GenerateMonthlyReport open={showMonthlyReportModal} onClose={() => setShowMonthlyReportModal(false)} />
)}
    </div>
  );
}

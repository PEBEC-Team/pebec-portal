...
export default function UserProfile() {
  ...
  const { user: clerkUser, openUserProfile } = useClerk();
  ...
  const userProfile = useQuery(api.users.getUserProfile, user ? { clerkUserId: user.id } : "skip");
  ...
  const industries = [
    ...
    "Manufacturing", "Real Estate", "Transportation", "Entertainment", "Other",
  ];
  ...
  const handleProfileImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (ev) => {
      ...
      if (clerkUser) await clerkUser.setProfileImage({ file });
    };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 overflow-x-hidden">
  ...
  <div className="flex justify-end items-center mb-4">
    <Button
      ...
      className="border-dashed border-emerald-600 text-emerald-600 hover:bg-emerald-50 transition"
    >
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-10">
        ...
        <div className="text-center mt-4">
         ...
         Primary email address: {user.primaryEmailAddress?.emailAddress}
            ...
            {/* Secondary emails (optional) */}
  {user.emailAddresses
    ?.filter((email) => email.id !== user.primaryEmailAddress?.id)
    .map((email) => (
      <p key={email.id} className="text-xs text-gray-400 italic">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
          <div>
            ...
            <Input className="w-full max-w-full" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} />
          </div>
          <div>
            <Label>Last Name</Label>
            <Input className="w-full max-w-full" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} />
          </div>
          <div>
  ...
  />
          ...
          <div>
  <Label>State</Label>
  {(role === "reform_champion" || role === "state_governor"|| role === "deputies") ? (
    <Input
      value={form.state}
      readOnly
      disabled
    ...
    <Select value={form.state} onValueChange={(value) => setForm({ ...form, state: value })}>
      ...
      <SelectContent className="w-full max-w-full">
        {statesOfNigeria.map((state) => (
          ...
          <div>
            <Label>Address</Label>
            <Input className="w-full max-w-full" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
          </div>
          ...
          {isUser && (
            <>
              <div>
                ...
                <Input className="w-full max-w-full" value={form.businessName} onChange={(e) => setForm({ ...form, businessName: e.target.value })} />
              </div>
              <div>
...
{/* Show password change only if no external accounts */}

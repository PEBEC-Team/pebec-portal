...
export default function UserOpenTickets() {
  ...
  const mdaUsers = useQuery(api.users.getUsersWithRole, { role: "mda" }) || [];
  ...
  if (!tickets)
    return <div className="text-gray-500 text-center py-6">Loading tickets...</div>;
  if (tickets.length === 0)
    return <div className="text-gray-500 text-center py-6">No open tickets found.</div>;
...
const handleCancelTicket = async () => {
  ...
  try {
    ...
    await cancelTicket({ ticketId: selectedTicket });

    toast({ title: "Ticket Canceled", description: "Your ticket will be deleted in 24 hours." });
    ...
    toast({ title: "Error", description: "Failed to cancel the ticket.", variant: "destructive" });
  } finally {
    setLoading(null);
...
const handleReopenTicket = async (ticketId: Id<"tickets">) => {
  ...
  try {
    ...
    toast({ title: "Ticket Reopened", description: "Your ticket is now active again." });
    ...
    toast({ title: "Error", description: "Failed to reopen the ticket.", variant: "destructive" });
  } finally {
    setLoading(null);
  }
};
...
async function sendEmails(ticket, action, mdaUsers) {
  const subject = action === "cancel"
    ? `Ticket #${ticket.ticketNumber} Canceled`
    : `Ticket #${ticket.ticketNumber} Reopened`;

  const message = action === "cancel"
    ? `<p>Your ticket <strong>#${ticket.ticketNumber}</strong> has been canceled.</p>`
    : `<p>Your ticket <strong>#${ticket.ticketNumber}</strong> has been reopened.</p>`;
  ...
  return (
    <section className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {tickets.map((ticket) => {
          const { text, color, progress, icon } = getStatusDetails(ticket.status);
          const hasUnread = hasUnreadNotifications(ticket._id);
          const assignedMdaName = getMdaName(ticket.assignedMDA);


          return (
            <div key={ticket._id} className="relative overflow-hidden rounded-2xl bg-white shadow-lg hover:shadow-xl transition-transform duration-300 hover:-translate-y-1 p-6">
              ...
              {hasUnread && (
                ...
                {/* Ticket Header */}
                <div className="flex items-center gap-4 mt-10"> {/* Increased margin-top */}
                ...
                <div className="mt-6 space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-sm text-gray-600">
                        {ticket.status === "open"
                          ...
                          ticket.status === "closed" ? "Your ticket has been cancelled and will be deleted within 24 hours"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-1.5 overflow-hidden rounded-full bg-gray-200">
                      <div className="h-full rounded-full bg-gradient-to-r from-green-500 to-teal-500" style={{ width: `${progress}%` }} />
                    </div>
                  </div>
  ...
  <Button 
    ...
    disabled={ticket.status === "closed" || loading === ticket._id}
  >
    {loading === ticket._id ? "Canceling..." : "Cancel"}
  </Button>
) : ticket.status === "closed" && !ticket.resolvedByAdmin ? ( 

  <Button onClick={() => handleReopenTicket(ticket._id)} disabled={loading === ticket._id}>
    {loading === ticket._id ? "Reopening..." : "Reopen"}
  </Button>
) : (

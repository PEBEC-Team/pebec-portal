...
const KanbanColumn = ({ title, status, tasks, updateTaskStatus, deleteTask, createTask }) => {
  ...
  return (
    <div className="bg-gray-100 p-4 rounded-lg shadow-md min-h-[400px] w-full border">
      ...
      <div className="space-y-3">
        {tasks.map((task) => (
          <KanbanCard key={task._id} task={task} updateTaskStatus={updateTaskStatus} deleteTask={deleteTask} />
        ))}
      </div>
    </div>
  );

...
const router = useRouter();
  ...
  const emailCheckResult = useQuery(
    ...
    form.email && form.email.includes("@") ? { email: form.email } : "skip"
  );
  ...
  switch (step) {
    case 1:
      ...
      case 2:
        ...
        return (
          form.assignedMDA.trim() !== "" &&
          form.incidentDate.trim() !== "" &&
          !checkFutureDate(form.incidentDate)
        );
...
`;
  ...
  useEffect(() => {
    if (
      ...
      form.verificationCode.split("").every((char) => /\d/.test(char)) &&
      !form.emailVerified
    ) {
      handleVerifyCode();
    }
...
{guestMode && emailCheckResult === true && (
  ...
  {guestMode && !form.emailVerified && (
    <div className="mt-2 space-y-3 transition-all">
 <Button
  variant="secondary"
  size="sm"
  onClick={handleSendVerificationCode}
  disabled={
    !form.email || !form.email.includes("@") || emailCheckResult === true
  }
>
...
<div className="flex flex-col space-y-1">
  ...
  <Select
    value={form.assignedMDA}
    onValueChange={(mda) => setForm({ ...form, assignedMDA: mda })}
  >
    <SelectTrigger>
  ...
  {Array.from(
    ...
    <SelectItem key={index} value={mdaName}>
      {mdaName}
    </SelectItem>
  ))}
</SelectContent>

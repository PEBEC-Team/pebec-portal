...
export default function Comments({ postId }: { postId: string }) {
  ...
  const comments = useQuery(api.posts.getCommentsByPost, { postId: postId as Id<"posts"> });
  ...
  const handleAddComment = async () => {
    ...
    try {
      if (clerkUserId) {
        await addComment({ postId: postId as Id<"posts">, content: commentText });
      } else if (guestName.trim() !== "") {
        await addComment({ postId: postId as Id<"posts">, content: commentText, guestName });
      } else {
        toast({
          title: "Error",
  ...
  const handleEditComment = async (commentId: string) => {
    ...
    try {
      await editComment({ commentId: commentId as Id<"comments">, content: editedContent });
    ...
    <div className="mt-10 max-w-2xl mx-auto">
      ...
      <div className="bg-white shadow-md rounded-lg p-4 mb-6">
        {clerkUserId ? (
          <Textarea
            ...
            rows={3}
            className="resize-none border border-gray-300 rounded-md p-3"
          />
        ) : (
          <>
            <input
      ...
      <ul className="space-y-4">
        {comments?.map((comment) => {
          ...
          return (
            <li key={comment._id} className="bg-gray-50 p-4 rounded-lg shadow-sm flex gap-4">
              {/* Avatar */}
                ...
                {editingComment === comment._id ? (
                  <div className="mt-2 flex gap-2">
                    <Textarea
                      ...
                      rows={2}
                      className="resize-none border p-2 rounded-md w-full"
                    />
              ...
              {/* Actions for Author (Only Authenticated Users Can Delete/Edit) */}
              {isCommentAuthor && !isGuestComment && (
      ...
      {/* ✅ ShadCN Toaster for displaying notifications */}
      <Toaster />
    </div>
  );
}

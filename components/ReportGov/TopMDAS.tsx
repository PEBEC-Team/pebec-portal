...
const podiumStyles = [
  'bg-gradient-to-t from-yellow-400 to-yellow-300 text-black h-64 z-20',
  'bg-gradient-to-t from-gray-400 to-gray-300 text-black h-56 z-10',
  'bg-gradient-to-t from-orange-500 to-orange-400 text-white h-48 z-0',
];
    ...
    return (
      <ResponsiveContainer width="100%" height={120}>
        <LineChart data={chartData} margin={{ top: 10, right: 20, bottom: 10, left: 0 }}>
          ...
          <YAxis tick={{ fontSize: 10 }} domain={[0, 'dataMax + 0.1']} />
          <Tooltip formatter={(val: any) => [`${val} hrs`, 'Time']} labelStyle={{ fontSize: 12 }} itemStyle={{ fontSize: 12 }} />
      ...
      <div className="bg-[#313132] text-white py-20 px-4 sm:px-8 rounded-3xl shadow-xl mb-24">
        ...
        <div className="flex flex-col sm:flex-row justify-center items-end gap-6">
          {top.map((mda, i) => (
            <div
              key={i}
              className={`relative flex flex-col items-center justify-end w-full sm:w-1/3 max-w-xs mx-auto rounded-t-3xl shadow-xl px-4 pt-20 pb-6 ${podiumStyles[i]} transform transition-all hover:scale-105`}
            >

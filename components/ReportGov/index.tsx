...
export default function ReportGovPage() {
  ...
  disabled={!!user && user.publicMetadata?.role !== "user"} 
...
>
                ...
                <span className="relative z-10 block px-6 py-3 rounded-2xl bg-green-950">
                  <div className="relative z-10 flex items-center space-x-3">
                    ...
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      className="w-7 h-7 transition-all duration-500 group-hover:translate-x-1.5 group-hover:text-lime-300"
                    >
      ...
      {/* Options Grid (Updated Labels) */}
      <div className="grid grid-cols-2 gap-6 text-white">
        <div className="flex flex-col items-center">
        <button
  onClick={handleFeedbackClick}
  disabled={!!user && user.publicMetadata?.role !== "user"}
  className={`w-[140px] h-[140px] bg-green-800 rounded-[140px_10px_10px_10px] shadow-xl flex items-center justify-center transition-all ${
    !!user && user.publicMetadata?.role !== "user"
      ? "opacity-50 cursor-not-allowed"
...
>
  ...
  disabled={!!user && user.publicMetadata?.role !== "user"}
  className={`w-[140px] h-[140px] bg-green-800 rounded-[10px_140px_10px_10px] shadow-xl flex items-center justify-center transition-all ${
    !!user && user.publicMetadata?.role !== "user"
      ? "opacity-50 cursor-not-allowed"
      : "hover:scale-105 hover:bg-green-700"
  }`}
...
<ReportGovFAQ />
      ...
      {/* Clerk Sign In Modal */}
      {showSignInModal && (
  <div className="fixed inset-0 z-50 bg-black bg-opacity-60 flex justify-center items-start pt-24">

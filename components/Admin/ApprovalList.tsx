...
const rolesRequiringMda = [
  "mda", "reform_champion", "saber_agent", "deputies", "magistrates", "state_governor",
];
const rolesRequiringState = [
  "reform_champion", "saber_agent", "deputies", "magistrates", "state_governor",
];
...
export default function InternalApprovals() {
  ...
  const filteredRequests = pendingRequests.filter((user) => {
    ...
    const matchesMda = !rolesRequiringMda.includes(roleFilter)
      || selectedMda === "all"
      || user.roleRequest?.mdaName === selectedMda;
    const matchesState = !rolesRequiringState.includes(roleFilter)
      || selectedState === "all"
      || user.roleRequest?.state === selectedState;
  ...
  const handleApprove = async (user: any) => {
    ...
    if (user.roleRequest.requestedRole === "mda" && mdaName) {
      const foundMda = mdasList.find(
        (mda) => mda.name.toLowerCase() === mdaName.toLowerCase()
      );
      if (foundMda) {
  ...
  const handleReject = async (user: any) => {
    try {
      await rejectRequest({ userId: user._id as Id<"users"> });

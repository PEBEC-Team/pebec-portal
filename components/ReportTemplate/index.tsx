...
export default function CreateReportTemplate({ existingTemplate, onClose }: CreateReportTemplateProps) {
  const { user } = useUser();
  const clerkUserId = user?.id ?? "";

  const convexUser = useQuery(api.users.getUserByClerkId, clerkUserId ? { clerkUserId } : "skip");
  ...
  const handleAddHeader = () => {
    ...
    setHeaders([...headers, { name: newHeader.trim(), type: headerType, options: headerType === "dropdown" ? dropdownOptions.split(",") : undefined }]);
    setNewHeader("");
    setDropdownOptions("");
  };
  ...
  const handleUpdateHeader = (index: number, updatedField: Partial<HeaderField>) => {
    setHeaders((prevHeaders) =>
      prevHeaders.map((header, i) => (i === index ? { ...header, ...updatedField } : header))
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (existingTemplate) {
        await updateTemplate({ id: existingTemplate.id, title, description, role, headers });
        toast.success("Template updated successfully!");
      } else {
        await createTemplate({ title, description, role, headers, createdBy: convexUserId });
      ...
      <div className="mt-4">
        ...
        {headers.map((header, index) => (
          <div key={index} className="flex gap-2 items-center mt-2">
            ...
            />
            <Select
              onValueChange={(val) => handleUpdateHeader(index, { type: val as HeaderField["type"] })}
              value={header.type}
            >
  ...
  <SelectItem value="date">Date</SelectItem> 
            ...
            {/* If dropdown is selected, show option input */}
            {header.type === "dropdown" && (
              <Input
                placeholder="Comma separated options"
                value={header.options?.join(", ") || ""}
                onChange={(e) => handleUpdateHeader(index, { options: e.target.value.split(",") })}
                className="w-1/3"
              />
            )}
    ...
    <label htmlFor={`date-${index}`} className="text-sm">
      {header.name}
        ...
        <div className="flex flex-col gap-2 mt-4">
          ...
          {headerType === "dropdown" && (
            <Input placeholder="Comma separated options" value={dropdownOptions} onChange={(e) => setDropdownOptions(e.target.value)} />
          )}

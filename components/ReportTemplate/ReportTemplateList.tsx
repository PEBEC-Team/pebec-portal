...
interface ReportTemplate {
  ...
  createdBy: Id<"users">;
  description?: string;
  headers: { name: string; type: "text" | "number" | "textarea" | "dropdown"| "checkbox"| "date"; options?: string[] }[];
}
...
export default function ReportTemplatesList() {
  ...
  const reportTemplates = useQuery(api.internal_reports.getReportTemplates, selectedRole === "all" ? {} : { role: selectedRole }) || [];
  ...
  const filteredReports = formattedTemplates
  .filter(
    (template) =>
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteTemplate({ id: selectedDeleteId });
      toast({
  ...
  const exportToExcel = (template: ReportTemplate) => {
    const worksheet = XLSX.utils.json_to_sheet(
      template.headers.map((header) => ({ "Column Name": header.name, Type: header.type }))
    );
      ...
      <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg shadow-md">
        <div className="w-full md:w-1/2">
          <Input
            ...
            className="w-full p-2 pl-10 text-gray-900 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:placeholder-gray-400"
          />
        </div>
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {filteredReports.length > 0 ? (
              ...
              <tr><td colSpan={4} className="px-4 py-3 text-center text-gray-500">No matching templates found.</td></tr>
            )}
          </tbody>
        </table>
      </div>

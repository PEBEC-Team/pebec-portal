...
const KanbanCard = ({ task, updateTaskStatus, deleteTask }) => {
  ...
  return (
    <div className={`relative p-4 rounded-lg shadow-lg border border-gray-300 hover:shadow-xl transition-all duration-200 ${isOverdue ? "bg-red-100 border-red-500" : "bg-white"}`}>
      ...
      <div className="w-full bg-gray-200 h-2 mt-3 rounded-full overflow-hidden">
        <div 
          className={`h-2 transition-all duration-500 ${progress === 100 ? "bg-green-500" : progress === 50 ? "bg-blue-500" : "bg-gray-400"}`} 
  ...
  <button 
    ...
    className="bg-gray-200 text-black mt-3 px-3 py-1 rounded-md w-full hover:bg-gray-300"
  >
    Change Status ▼
  </button>

  {statusMenuOpen && (
    <div className="absolute top-full left-0 z-50 bg-white shadow-md p-2 rounded-md mt-1 w-full border">
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent className="max-w-sm w-full">
          ...
          <div className="flex justify-end gap-2">
            <button 
              onClick={() => { deleteTask({ taskId: task._id }); setDeleteConfirmOpen(false); }} 

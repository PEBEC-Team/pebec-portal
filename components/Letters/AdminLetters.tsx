...
const ROLES_WITH_STATE_SELECTION = [
  "reform_champion",
  "deputies",
  "magistrates",
  "state_governor",
...
export default function SubmitLetterForm({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 overflow-auto flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md relative max-h-[90vh] overflow-y-auto">
        ...
        {ROLES_WITH_STATE_SELECTION.includes(selectedRole) && (
          <select
            value={selectedState}
            onChange={(e) => {
      ...
      <option key={user._id} value={user._id}>
        ...
        {selectedUserDetails && (
          <div className="flex items-center gap-3 border p-3 rounded-lg bg-gray-50 mb-4">
            <Image
              src={selectedUserDetails.imageUrl || "/default-avatar.png"}
              alt="User"
              width={36}
              height={36}
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">
                {selectedUserDetails.jobTitle
                  ? `${selectedUserDetails.jobTitle}, `
                  : ""}
                {selectedUserDetails.staffStream ||
                  selectedUserDetails.mdaName ||

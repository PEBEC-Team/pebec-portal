...
const Header = () => {
  ...
  const handleSignOut = async () => {
    toast("Signing out...", { duration: 500 });
 ...
 const dashboardLink =
  userRole === "admin"
    ...
    : userRole === "mda"
    ? "/mda"
    : userRole === "staff" && staffStream === "investments"
    ? "/staff/projects"
    : userRole === "staff" && staffStream === "receptionist"
    ? "/staff/letters"
    : userRole === "staff"
    ? "/staff"
    : userRole === "reform_champion"
    ? "/reform_champion"
    : userRole === "deputies"
    ? "/deputies"
    : userRole === "magistrates"
    ? "/magistrates"
    : userRole === "state_governor"
    ? "/state_governor"
    : userRole === "vice_president"
    ? "/vice_president"
    : userRole === "president"
    ? "/president"
    : userRole === "receiptionist"
    ? "/receiptionist"
    : userRole === "account"
    ? "/account"
    : userRole === "auditor"
    ? "/auditor"
     : userRole === "saber_agent"
    ? "/saber_agent"
    : "/reportgov";
  ...
  return (
    <header
      className={`fixed left-0 top-0 w-full py-6 transition duration-200 ${
        stickyMenu ? "bg-white !py-4 shadow dark:bg-black" : "bg-transparent"
      } ${modalOpen ? "pointer-events-none opacity-50" : "z-50"}`}
    >
      <div className="relative mx-auto max-w-7xl flex items-center justify-between px-4 md:px-8 xl:px-0">
        ...
        {/* Mobile Menu */}
     {/* Mobile View: Reportgov + SABER + Menu */}
<div className="xl:hidden flex items-center gap-2">
  {/* reportgov.ng button */}
  <button
    onClick={handleReportGovClick}
    className="flex items-center rounded-full bg-[#FF3226] text-white text-xs font-bold px-2.5 py-1.5"
  >
    reportgov
  ...
  <Link
    href="/saber"
    className="flex items-center gap-1 rounded-full bg-[#2563EB] text-white text-xs font-bold px-3 py-1.5"
  >
        ...
        <nav className="hidden xl:flex ">
          <ul className="flex space-x-6 text-gray-700 dark:text-white text-[15px] font-medium">
            ...
            <li>
              <a
                href="/"
                className={`hover:text-[#2D8B10] dark:hover:text-gray-300 ${
                  pathUrl === "/" ? "text-[#2D8B10] font-semibold" : ""
                }`}
              >
                Home
              </a>
...
>
  ...
  {activeDropdown === "about" && (
    <div
      className="absolute left-[-120px] top-[60px] w-[900px] bg-white shadow-xl rounded-xl py-8 px-10 grid grid-cols-3 gap-10 border border-gray-200 z-50"
    >
      {/* Menu Items */}
      <div className="col-span-2 grid grid-cols-2 gap-x-10 gap-y-8">
        {[
          { title: "Overview", description: "Discover the vision and goals of PEBEC.", icon: MdDashboard, path: "/overview" },
          { title: "Members", description: "Meet the people behind the transformation.", icon: MdPeople, path: "/members" },
          { title: "Reforms", description: "Explore ongoing reforms and initiatives.", icon: MdAssignment, path: "/reforms" },
          { title: "Contact Us", description: "Get in touch with us!", icon: MdAnalytics, path: "/support" },
        ].map(({ title, description, icon: Icon, path }) => (
      ...
      {/* Right Box */}
      <div
        className="relative flex flex-col justify-center items-center rounded-lg p-8 text-white text-center shadow-lg hover:scale-[1.02] transition-all duration-300"
        style={{
...
>
  ...
  {activeDropdown === "resources" && (
    <div
      className="absolute left-0 top-full w-[480px] bg-white shadow-lg rounded-md py-5 px-6 grid grid-cols-2 gap-4 z-50"
    >
      {[
        { title: "Reports", path: "/reports", icon: MdLibraryBooks },
        { title: "FAQ", path: "/faq", icon: MdContactMail },
        { title: "Frameworks", path: "/frameworks", icon: MdAssignment },
        { title: "Downloads", path: "/downloads", icon: MdAnalytics },
        { title: "Gallery", path: "/media", icon: FaCameraRetro },
      ].map((item, key) => (
        <Link
          key={key}
          href={item.path}
        ...
        >
          ...
          <div>
            ...
            <li>
              <a
                href="/portal"
                className={`hover:text-[#2D8B10] dark:hover:text-gray-300 ${
                  pathUrl === "/portal" ? "text-[#2D8B10] font-semibold" : ""
                }`}
              >
                ePortal
              </a>
            ...
            <li>
              <a
                href="/events"
                className={`hover:text-[#2D8B10] dark:hover:text-gray-300 ${
                  pathUrl === "/events" ? "text-[#2D8B10] font-semibold" : ""
                }`}
              >
                Events
              </a>
          ...
          {/* User & ReportGov Button */}
{/* User & ReportGov Button - Only visible on xl and above */}
<div className="hidden xl:flex items-center gap-3">
  ...
  <div className="flex items-center gap-3 relative">
    ...
    <Menu as="div" className="relative">
      ...
      {/* Absolutely positioned dropdown - does NOT move the layout */}
      <Transition
        as={Fragment}
    ...
    {/* User Role Text (always visible) */}
    <Link
      href={dashboardLink}
      className="text-sm font-medium text-gray-800 hover:text-green-700 transition"
    >
      {userRoleText}
    </Link>
  </div>
  ...
  {/* Reportgov & Saber Grouped Buttons */}
  <div className="flex overflow-hidden text-sm font-semibold rounded-full shadow-md">
    <button
      onClick={handleReportGovClick}
      className="bg-[#FF3226] hover:bg-[#e02c21] text-white flex items-center gap-1 px-4 py-2 transition duration-200"
    >
      reportgov
    ...
    <Link
      href="/saber"
      className="bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-2 px-4 py-2 transition duration-200"
    >

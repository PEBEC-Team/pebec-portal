...
interface TicketProps {
  ...
  qrCodeUrl: string;
  userAnswers: { questionText: string; answer: string }[];
  ticketOwner?: string;
}

const Ticket: React.FC<TicketProps> = ({ ticketNumber, event, qrCodeUrl, userAnswers, ticketOwner }) => {
  return (
    <div className="relative bg-white shadow-lg rounded-lg border border-gray-300 overflow-hidden flex flex-col md:flex-row w-full md:w-[1200px] h-auto md:h-auto p-2 md:p-4 md:mb-10">
      {/* Left Section (Cover Image + Logo) */}
      <div className="relative w-full md:w-1/3 flex flex-col items-center justify-center p-2 md:p-4">
        ...
        onLoad={() => console.log("✅ Image loaded:", event?.coverImageUrl)}
        onError={() => console.error("❌ Image failed to load:", event?.coverImageUrl)}
        alt="Event Cover" 
        className="w-full h-full object-cover md:rounded-l-lg opacity-80"
      />
      ...
      <div className="flex flex-col justify-between p-4 bg-gray-900 text-white w-full md:w-2/3">
        <div className="flex justify-between items-center">
          <div>
            ...
            {/* ✅ Show ticket owner only if it exists */}
  ...
  <p className="text-gray-300">
    Time: {new Date(event.eventDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}
  </p>
</div>

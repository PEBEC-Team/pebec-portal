...
export default function GenerateTicketReport({ open, onClose }: Props) {
...
"Title",
      ...
      "Status", "Submission Date", "State", "Address", "Email", "Phone", "Description", "Resolution Note",
    ];
  ...
  const downloadExcel = () => {
    ...
    const columnHeaders = [
      "TicketNumber", "Name",   "BusinessName",
"Title",
      ...(showMdaColumn ? ["MDA"] : []),
      "Status", "SubmissionDate", "State", "Address", "Email", "Phone", "Description", "ResolutionNote"
    ];
  

    const worksheet = XLSX.utils.json_to_sheet([{ A: filterHeader }, {}, ...dataRows], {
      skipHeader: true,
    });
  ...
  (step === 2 && !(
    ...
    (filterMode === "business" && selectedBusiness) ||
    (filterMode === "mda" && selectedMDA)
  )) ||
  ...
  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
    <div
      ...
      className={`cursor-pointer border p-4 rounded-lg text-center shadow-sm hover:shadow-md transition ${
        filterMode === "user" ? "ring-2 ring-blue-500" : ""
      }`}
    >
    ...
    <div
      ...
      className={`cursor-pointer border p-4 rounded-lg text-center shadow-sm hover:shadow-md transition ${
        filterMode === "business" ? "ring-2 ring-blue-500" : ""
      }`}
    >
...
{showMdaFilter && mdas && (
  <div
    ...
    className={`cursor-pointer border p-4 rounded-lg text-center shadow-sm hover:shadow-md transition ${
      filterMode === "mda" ? "ring-2 ring-blue-500" : ""
    }`}
  >
      ...
      <PopoverTrigger asChild>
        <Button variant="outline" className="w-full justify-between">
          {selectedBusiness === "all"
            ? "All Businesses"
            : selectedBusiness || "Choose business"}
          <ChevronsUpDown className="ml-2 h-4 w-4 opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-full p-0">
        ...
        <Button variant="outline" className="w-full justify-between">
          {selectedUser === "all"
            ...
            : users?.find((u) => u._id === selectedUser)?.firstName +
                " " +
                users?.find((u) => u._id === selectedUser)?.lastName || "Choose a user"}
          <ChevronsUpDown className="ml-2 h-4 w-4 opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-full p-0">

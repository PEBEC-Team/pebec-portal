...
export default function BusinessLettersAdmin() {
  ...
  const letters = useQuery(api.business_letters.getAllBusinessLetters, { refreshKey }) || [];
...
const [activeLetterFiles, setActiveLetterFiles] = useState<{
  ...
  const staffStreams = useMemo(() => {
    ...
    [
      "regulatory", "innovation", "judiciary",
      "communications", "investments",
      "receptionist", "account", "auditor",
    ].forEach(stream => streams.add(stream));
  ...
  .filter((l) => {
    ...
    const matchesDate =
      (!filter.from || new Date(filter.from) <= date) &&
  ...
  const handleDelete = async () => {
    if (selectedLetter) {
      await deleteLetter({ letterId: selectedLetter });
      toast({
  ...
  const handleAssignConfirm = async () => {
    const streamStaff = users.filter(
      (u) => u.role === "staff" && u.staffStream === selectedStream
    );
  
    const selectedStaff = streamStaff.filter((staff) =>
    ...
    try {
      ...
      await assignLettersToStaff({
        ...
        staffIds: assignEntireStream
          ? streamStaff.map((s) => s._id)
          : selectedStaff.map((s) => s._id),
        staffNames: assignEntireStream
          ? streamStaff.map((s) => `${s.firstName ?? ""} ${s.lastName ?? ""}`.trim())
          : selectedStaff.map((s) => `${s.firstName ?? ""} ${s.lastName ?? ""}`.trim()),
      });
  ...
  const openAssignDialog = () => {
    ...
    const selectedLetters = letters.filter(letter =>
      selectedLetterIds.includes(letter._id)
    );
  
    const commonStream = selectedLetters.every(
      (l) => l.assignedStream === selectedLetters[0].assignedStream
    )
      ? selectedLetters[0].assignedStream
      : "";
        ...
        selectedLetters.some(
          (l) =>
            ...
            (Array.isArray(l.assignedTo)
              ? l.assignedTo.includes(user._id)
              : l.assignedTo === user._id) &&
            user.staffStream === commonStream
        )
      )
      .map((u) => u._id);
  ...
  <div className="flex justify-end mb-4">
    <Button onClick={openAssignDialog}>
      {letters.some(
        (l) =>
          selectedLetterIds.includes(l._id) &&
          (l.assignedStream || l.assignedTo?.length)
      )
        ? "Reassign Selected"
        : "Assign Selected"}
    </Button>
...
<div className="flex justify-end mb-4">
      ...
      {paginatedLetters.map((letter) => (
        <tr key={letter._id} className="border-t">
          <td className="p-3">
            <input
              type="checkbox"
              checked={selectedLetterIds.includes(letter._id)}
              onChange={() => toggleSelection(letter._id)}
            />
          </td>
          ...
          <td className="p-3">
            {letter.assignedStream ? (
              <span className="capitalize">{letter.assignedStream}</span>
            ) : (
  ...
  {/* Stream Selection (still single select) */}
  <select
...
>
  ...
  <input
    type="checkbox"
    id="assignAll"
    checked={assignEntireStream}
    onChange={() => setAssignEntireStream(!assignEntireStream)}
  />
  ...
  {/* Multi-select staff with checkboxes */}
  {selectedStream && !assignEntireStream && (
  <div className="border rounded p-3 max-h-60 overflow-y-auto">
    <p className="text-sm font-medium mb-2 text-gray-700">
      Select Staff in "{selectedStream}"
    </p>
    {users
      .filter((u) => u.role === "staff" && u.staffStream === selectedStream)
      .map((staff) => (
    ...
    {users.filter((u) => u.role === "staff" && u.staffStream === selectedStream).length === 0 && (
      <p className="text-sm text-gray-500 italic">No staff in this stream.</p>
    )}
  </div>
)}

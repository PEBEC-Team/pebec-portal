...
const getRolePath = () => {
      ...
      <Popover open={notificationsOpen} onOpenChange={toggleNotifications}>
        <PopoverTrigger asChild>
          <button className="relative p-3 rounded-full bg-white shadow-md border border-gray-300 hover:bg-gray-100 transition duration-200">
            ...
            {unreadCount > 0 && (
              <div
                className={`absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-bold shadow-md ${pulseAnimation}`}
              >
  ...
  {notifications.length === 0 ? (
    <div className="text-gray-500 text-sm text-center py-4">No new notifications</div>
  ) : (
    notifications.slice(0, 7).map((notification) => (
      <div
        key={notification._id as string}
        className="flex justify-between items-center p-3 border-b border-gray-200 hover:bg-gray-50 rounded-md"
      >
        <div className="flex-1">
          ...
          {notification.ticketId && (
            <Link href={`/${getRolePath()}/tickets/${notification.ticketId}`} passHref>
          ...
          {notification.postId && (
            <Link href={`/posts/${notification.postId}`} passHref>
              <Button variant="link" className="text-blue-600 text-xs hover:underline">
                Go to Post →
              </Button>
            </Link>
        ...
        <button
          onClick={() =>
            markAsReadMutation({ notificationId: notification._id as Id<"notifications"> })
          }
        ...
        <button
          ...
          className="text-gray-400 hover:text-red-500 transition duration-200 ml-2"
        >
          ✖
        </button>
      </div>
  ...
  <Link href="/notifications">
    <Button
      variant="outline"
      className="w-full mt-4 text-sm font-medium text-blue-600 border-blue-600 hover:bg-blue-50 hover:text-blue-800"
    >
      View All Notifications →
    </Button>
  </Link>

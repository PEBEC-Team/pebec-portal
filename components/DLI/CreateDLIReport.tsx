...
export default function CreateDLIForm({ onClose, existingDli }: Props) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full relative overflow-y-auto max-h-[90vh]">
        ...
        <Input
          placeholder="DLI Title"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          className="mb-3"
        />
        <Textarea
          placeholder="DLI Description (Optional)"
          value={description}
        ...
        {fileName && (
          <p className="text-sm text-gray-600 mt-2 mb-4">Attached file: {fileName}</p>
        )}

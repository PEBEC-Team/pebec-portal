...
export default function SubmitDLIReport({ templateId }: { templateId: Id<"dli_templates"> }) {
  const dliTemplate = useQuery(api.dli.getDliTemplateById, { id: templateId });
  ...
  const handleAnswerChange = (stepIndex: number, qIndex: number, value: string | Id<"_storage">) => {
    setAnswers((prevAnswers) => {
      ...
      if (!newAnswers[stepIndex]) {
        newAnswers[stepIndex] = { stepTitle: steps[stepIndex].title || "", responses: [] };
      }
  ...
  const handleSubmit = async () => {
    await submitDliReport({ dliTemplateId: templateId, answers });
          ...
          <Step key={stepIndex}>
            ...
            {step.questions.map((q, qIndex) => (
              <div key={qIndex} className="mb-4">
                ...
                {/* ✅ Correct Field Rendering Based on Admin Selection */}
                {q.type === "text" && (
                  <Input onChange={(e) => handleAnswerChange(stepIndex, qIndex, e.target.value)} />
                )}
                {q.type === "textarea" && (
                  <Textarea onChange={(e) => handleAnswerChange(stepIndex, qIndex, e.target.value)} />
                )}
                {q.type === "dropdown" && (
                  <select onChange={(e) => handleAnswerChange(stepIndex, qIndex, e.target.value)} className="w-full p-2 border rounded-md">

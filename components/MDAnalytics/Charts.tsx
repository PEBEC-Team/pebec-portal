  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      {statsData.map((stat, index) => (
        <div key={index} className="relative bg-white p-6 rounded-lg shadow-md flex flex-col justify-between">
          ...
          {/* ✅ Progress Bar for Report Progress */}
          {stat.progressBar && (
            <div className="mt-4">
          ...
          {/* ✅ Message for Score */}
          {stat.message && (
            <p className="text-sm text-gray-600 mt-2">{stat.message}</p>
          )}
        </div>
      ))}
    </div>

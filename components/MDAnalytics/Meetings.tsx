...
export default function MDAMeetingsAnalytics() {
  ...
  const filteredSlots = useMemo(() => {
    return allSlots.filter(
      (slot) => slot.bookedBy === currentUser?._id
    );
  }, [allSlots, currentUser]);

  const thisWeeksMeetings = filteredSlots.filter((slot) =>
    isSameWeek(parseISO(`${slot.date}T${slot.startTime}`), today, { weekStartsOn: 1 })
  );

  const nextWeeksMeetings = filteredSlots.filter((slot) =>
    isSameWeek(parseISO(`${slot.date}T${slot.startTime}`), addDays(today, 7), { weekStartsOn: 1 })
  );
  ...
  const MeetingItem = ({ slot }: { slot: (typeof allSlots)[number] }) => {
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1 bg-gray-50 border rounded-2xl p-6 shadow-sm space-y-6">
          ...
          {thisWeeksMeetings.length > 0 ? (
            ...
            <p className="text-sm text-gray-400">No meetings for this week.</p>
          )}
        </div>

        {isEndOfWeek && nextWeeksMeetings.length > 0 && (
          <div className="lg:w-1/3 bg-gray-50 border rounded-2xl p-4 shadow-sm space-y-4">

...
export default function MdaTicketsChart() {
  ...
  const handleDownloadPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: monthLabels.map((month, index) => [
        month,
        ticketsPerMonth[index],
        resolvedPerMonth[index],
        inProgressPerMonth[index],
        closedPerMonth[index],
        pendingPerMonth[index],
      ]),
    });
    ...
    autoTable(doc, {
      ...
      head: [["Category", "Count"]],
      body: [
        ["Total Tickets Assigned", incidentsStats.totalAssigned],
        ["Resolved Reports", incidentsStats.resolved],
        ["In Progress Reports", incidentsStats.unresolved],
        ["Closed Reports", incidentsStats.resolvedPercentage + "%"],
      ],
    });
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg w-full relative">

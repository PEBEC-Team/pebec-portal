...
export default function FillReportForm({ template, onClose }) {
  const { user } = useUser();
      ...
      {template.headers.map((header, index) => (
        <div key={index} className="mb-3">
          ...
          {header.type === "textarea" ? (
            <Textarea value={formData[index]} onChange={(e) => handleChange(index, e.target.value)} />
          ) : (
            <Input type={header.type === "number" ? "number" : "text"} value={formData[index]} onChange={(e) => handleChange(index, e.target.value)} />
          )}

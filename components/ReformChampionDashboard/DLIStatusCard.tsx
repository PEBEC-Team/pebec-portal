...
export default function DLIStatusCard({ state }: { state: string }) {
  const [openModal, setOpenModal] = useState(false);
  const stateDLIs = useQuery(api.dli.getStateDLIsReformChampion, { state });

  const activeCount =
    stateDLIs?.filter(
      (dli) => dli.status === "in_progress" || dli.status === "completed"
    ).length || 0;

  ...
  export default function ViewLettersPage() {
    ...
    const allUsers = useQuery(api.users.getUsers) || [];    const getFileUrl = useMutation(api.letters.getLetterFileUrl);
    ...
    const paginatedLetters = sortedLetters.slice(
      ...
      allUsers.map((user) => [
        user._id,
        `${user.firstName || ""} ${user.lastName || ""} (${user.role || "N/A"})`.trim(),
      ])
    );

    useEffect(() => {
      const fetchFileUrls = async () => {
        const urls: { [key: string]: { url: string; fileName: string } } = {};    
    ...
    <Button
      ...
      onClick={async () => {
        ...
        try {
          ...
          URL.revokeObjectURL(blobUrl);        } catch (err) {

...
export default function BFAReportsPage() {
  ...
  const [fileUrls, setFileUrls] = useState<{ [key: string]: { url: string; fileName: string } }>({});
...
monthStart.setHours(0, 0, 0, 0);
  ...
  const uniqueMdas = Array.from(
    new Set(submittedReports.map((r) => r.mdaName).filter(Boolean))
  );
  ...
  if (
    userRole !== "staff" ||
    !userStream ||
    !allowedStreams.includes(userStream)
  ) {
    return (
    ...
    const fetchFileUrls = async () => {
          ...
          const urls = await Promise.all(
            submittedReports.map(async (report) => {
              if (report.fileId) {
                ...
                const result = await getFileUrl({ storageId: report.fileId });
      ...
      if (result && typeof result === "object" && result.url) {
        return {
          ...
          fileName:
            result.fileName ||
            `BFA_Report_${report.userName?.replace(/\s+/g, "_") || "unknown"}_${new Date(report.submittedAt).toISOString().split("T")[0]}.pdf`,
        };
      }
    }
              return null;
  ...
  const exportToPDF = (report) => {
    ...
    doc.text(
      `Report of ${report.mdaName ?? "Unknown"} (${report.userName ?? "Unknown"})`,
...
{/* <Button variant={activeQuickFilter === "month" ? "default" : "outline"} onClick={() => {
  applyDateRange(monthStart, new Date());
  ...
  This Month
        ...
        <Button variant="ghost" onClick={() => {
          setStartDate(""); setEndDate(""); setActiveQuickFilter(null);
  ...
  <div className="flex items-center justify-center space-x-2">
    {report.fileId && !report.templateId && fileUrls[report.fileId] ? (
      <a
        href={fileUrls[report.fileId].url}
        download={fileUrls[report.fileId].fileName || "report.pdf"}
        target="_blank"
      ...
      <>
        ...
        <Button
          ...
          size="sm"
          variant="ghost"
          className="p-1.5 text-green-600 hover:bg-green-100"
        >

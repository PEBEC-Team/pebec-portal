...
import {
  ...
  FaMapMarkerAlt, FaBuilding, FaCalendarAlt, FaTrashAlt, FaBriefcase
} from "react-icons/fa";
...
export default function AdminTicketDetailsPage() {
  ...
  const ticket = useQuery(
    ...
    ticketId && !isDeleted ? { ticketId: ticketId as Id<"tickets"> } : "skip"
  );

  const ticketUser = useQuery(
    api.users.getUserByClerkId,
    ticket?.clerkUserId ? { clerkUserId: ticket.clerkUserId } : "skip"
  );
  ...
  const basePath = roleBasePath[role as keyof typeof roleBasePath] || "admin";
  ...
  function handleStatusChange(newStatus: "open" | "in_progress" | "resolved" | "closed") {
    ...
    if (newStatus === "resolved" || newStatus === "closed") {
      ...
      updateTicketStatus({ ticketId: ticketId as Id<"tickets">, status: newStatus })
        .then(() => toast.success(`✅ Ticket updated to ${newStatus.replace("_", " ")}`))
  ...
  async function handleDeleteTicket() {
    try {
      await deleteTicket({ ticketId: ticketId as Id<"tickets"> });
    ...
    <FaTrashAlt className="w-4 h-4" /> Delete Ticket
      ...
      <div className="mt-6 p-5 bg-white shadow-md rounded-lg border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700 text-sm">
          ...
          {ticketUser?.businessName && (
            <div className="flex items-center gap-2">
              <FaBriefcase className="text-gray-600" />
              <span><strong>Business:</strong> {ticketUser.businessName}</span>
            </div>
          )}
          <div className="flex items-center gap-2">

...
export default function ProjectDetailPage() {
  ...
  const handleToggleStep = async (index: number) => {
    await toggleStep({ projectId: project._id, index });
    ...
    if (newStatus !== project.status) {
      await updateStatus({ projectId: project._id, status: newStatus });
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (status === "open")
      return <span className={`${base} bg-gray-100 text-gray-700`}>Open</span>;
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div
          ref={detailsRef}
          className="relative flex-1 bg-white rounded-xl shadow border border-gray-100 p-6 min-h-[400px]"
        >
          {isOwner && (
            <Button
              variant="ghost"
        ...
        {/* Steps Card */}
        <div
          className="flex-1 bg-white rounded-lg shadow p-6 overflow-y-auto"
          style={{ maxHeight: detailsHeight ?? "auto" }}
        >
            ...
            <div className="flex flex-col sm:flex-row gap-2 mt-4">
              ...
              <Button
                onClick={async () => {
                  ...
                  await addStep({ projectId: project._id, title: newStep });
                  setNewStep("");
                }}
              >
                Add Step
          ...
          <div className="mt-4 flex flex-col sm:flex-row gap-2">
            ...
            <Button
              onClick={async () => {
                ...
                await addUpdate({ projectId: project._id, text: newUpdate });
                setNewUpdate("");
              }}
            >
              Post
            ...
            <Button
              ...
              onClick={async () => {
                if (confirmStepIndex !== null) {
                  await deleteStep({ projectId: project._id, index: confirmStepIndex });

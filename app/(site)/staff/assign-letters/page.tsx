...
export default function AssignLettersPage() {
  ...
  const userMap = Object.fromEntries(
    allUsers.map((u) => [u._id, `${u.firstName} ${u.lastName}`])
  );

  const auditorStaff = allUsers.filter(
    (u) => u.role === "staff" && u.staffStream === "auditor"
  );
  ...
  const handleStatusChange = async (
    ...
    await updateStatus({ letterId: id, status });
  };
...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
  ...
  <select
    value={filters.role}
    onChange={(e) => setFilters({ ...filters, role: e.target.value, sender: "" })}
    className="border p-2 rounded"
  >
    <option value="">All Roles</option>
    {Array.from(new Set(allUsers.map((u) => u.role).filter(Boolean))).map((role) => (
    ...
    <TableBody>
      {paginated.map((letter) => {
        const assignedToName =
          allUsers.find((u) => u._id === (letter as any).assignedTo)?.firstName +
            " " +
          allUsers.find((u) => u._id === (letter as any).assignedTo)?.lastName;
        ...
        return (
          <TableRow key={letter._id}>
            ...
            <TableCell>
              {assignedToName && assignedToName !== "undefined undefined"
                ? assignedToName
                : "-"}
            </TableCell>
            <TableCell>
              <div className="flex flex-col gap-2 md:flex-row justify-center items-center">
                ...
                {/* Auditor's status update dropdown */}
                {userRole === "staff" && userStream === "auditor" && (
                  <select
                    ...
                    onChange={(e) =>
                      handleStatusChange(
                        letter._id,
                        e.target.value as "acknowledged" | "in_progress" | "resolved"
                      )
                    }
                  >
      ...
      <Dialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen}>
        <DialogContent>
          ...
          <select
            ...
            onChange={(e) => setSelectedAuditor(e.target.value as Id<"users">)}
          >
            <option value="">Select Auditor</option>
            {auditorStaff.map((staff) => (

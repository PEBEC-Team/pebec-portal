...
export default function AdminNewsletterDashboard() {
    ...
    (async () => {
      ...
      for (const n of newsletters.newsletters) {
        if (n.attachmentId && !attachmentLinks[n.attachmentId]) {
          const url = await getFileUrl({ storageId: n.attachmentId });
          if (url) newLinks[n.attachmentId] = url;
        }
      }
      setAttachmentLinks((prev) => ({ ...prev, ...newLinks }));
    })();
  ...
  useEffect(() => {
    if (attachmentId) {
      (async () => {
        const url = await getFileUrl({ storageId: attachmentId });
  ...
  const handleCreate = async () => {
    ...
    await createNewsletter({ subject, message, attachmentId });
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="overflow-auto border rounded-md shadow-sm bg-white">
        <table className="w-full text-sm table-auto">
          ...
          <tbody>
          {newsletters?.newsletters && newsletters.newsletters.length > 0 ? (
  newsletters.newsletters.map((n) => (
                <tr key={n._id} className="border-t hover:bg-gray-50 transition">
                  <td className="px-4 py-3 font-medium text-gray-800">{n.subject}</td>
                  <td className="px-4 py-3 text-gray-600">
                    {format(new Date(n.createdAt), "PPpp")}
                  </td>
                  <td className="px-4 py-3">
                    {n.attachmentId && attachmentLinks[n.attachmentId] ? (
                      <a
                        href={attachmentLinks[n.attachmentId]}
                        target="_blank"
      ...
      <div className="flex justify-center items-center gap-4 mt-6">
        ...
        <Button
          ...
          disabled={newsletters && newsletters.newsletters.length < 20}
        >
          Next
        </Button>
      </div>

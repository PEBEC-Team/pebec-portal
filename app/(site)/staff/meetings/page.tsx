...
export default function StaffAvailabilityPage() {
  ...
  const availability = useQuery(
    api.meetings.getStaffAvailability,
    convexUser?._id && convexUser?.staffStream
      ? { userId: convexUser._id, workstream: convexUser.staffStream }
      : "skip"
  ) || [];
  ...
  const [showDeleteDialog, setShowDeleteDialog] = useState<{ open: boolean; slotId: Id<"availability"> | null }>({ open: false, slotId: null });
  ...
  const [releaseDialog, setReleaseDialog] = useState<{ open: boolean; slotId: Id<"availability"> | null }>({ open: false, slotId: null });
  ...
  useEffect(() => {
    availability.forEach(async (slot) => {
      ...
      if (isBefore(start, new Date()) && !slot.deactivated) {
        await deactivateSlot({ slotId: slot._id, deactivate: true });
      }
    });
  }, [availability]);
  ...
  const visibleSlots = useMemo(() => {
    ...
    if (activeTab === "upcoming") {
      return availability.filter(slot => {
        ...
        return (
          slot.bookedBy &&
          !(slot as any)?.cancelled &&
          isAfter(start, now) &&
          (showAllMeetings || (selectedDate && isSameDay(start, selectedDate)))
        );
      });
    } else {
      return slotsForDay;
        ...
        <div className="flex-1 bg-white p-4 rounded-xl shadow max-h-[600px] overflow-y-auto">
            ...
            <h2 className="text-lg font-semibold">
              {activeTab === "upcoming"
                ? showAllMeetings
                  ...
                  : `📋 Meetings for ${format(selectedDate ?? new Date(), "EEEE, MMM d")}`
                : `⚙️ Slot Settings for ${format(selectedDate ?? new Date(), "EEEE, MMM d")}`}
            </h2>
          ...
          <div className="space-y-3">
            {visibleSlots.map((slot) => {
              ...
              return (
                <div key={slot._id} className="border rounded-lg px-4 py-3 flex flex-col sm:flex-row sm:justify-between gap-3 bg-gray-50">
                  <div>
  ...
  <>
    <Switch
      checked={!slot.deactivated}
      onCheckedChange={async () => {
        setLoadingIds(prev => [...prev, slot._id]);
        await deactivateSlot({ slotId: slot._id, deactivate: !slot.deactivated });
    ...
    <Button
      variant="destructive"
      size="sm"
      onClick={() => setShowDeleteDialog({ open: true, slotId: slot._id })}
    >
      Delete
    </Button>
  </>
                      ...
                      {slot.bookedBy && (
                        <Button
                          variant="destructive"
                          size="sm"
                          onClick={() => setReleaseDialog({ open: true, slotId: slot._id })}
                        >
                          Release
                        </Button>
                      )}
            ...
            {visibleSlots.length === 0 && (
              <p className="text-gray-500 text-sm">No {activeTab === "upcoming" ? "appointments." : "slots for this day."}</p>
            )}
            ...
            {activeTab === "slots" && slotsForDay.length > 0 && (
              <div className="text-right mt-4">
                <ToggleSwitch
                  checked={slotsForDay.every((s) => !s.deactivated)}
                  onChange={async () => {
                    const shouldActivate = slotsForDay.every((s) => s.deactivated);
                    await Promise.all(
                      slotsForDay.map((slot) =>
                        deactivateSlot({ slotId: slot._id, deactivate: !shouldActivate })
                      )
                    );
...
/>
  ...
  <div>
    ...
    />
      ...
      <Dialog open={releaseDialog.open} onOpenChange={() => setReleaseDialog({ open: false, slotId: null })}>
        <DialogContent>
          ...
          <DialogFooter className="mt-4 flex gap-2">
            <Button variant="outline" onClick={() => setReleaseDialog({ open: false, slotId: null })}>No</Button>
            <Button variant="destructive" onClick={async () => {
              if (releaseDialog.slotId) {
                await cancelSlot({ slotId: releaseDialog.slotId });
                toast.success("Slot released");
                setReleaseDialog({ open: false, slotId: null });
              }
            }}>Yes, Release</Button>
          </DialogFooter>
        </DialogContent>
...
<Dialog open={showDeleteDialog.open} onOpenChange={() => setShowDeleteDialog({ open: false, slotId: null })}>
  <DialogContent>
    <DialogHeader><DialogTitle>Confirm Delete</DialogTitle></DialogHeader>
    <p className="text-sm text-gray-600">Are you sure you want to delete this slot? This action cannot be undone.</p>
    <DialogFooter className="mt-4 flex gap-2">
      <Button variant="outline" onClick={() => setShowDeleteDialog({ open: false, slotId: null })}>Cancel</Button>
      <Button variant="destructive" onClick={async () => {
        if (showDeleteDialog.slotId) {
          await deleteSlot({ slotId: showDeleteDialog.slotId });
          toast.success("Slot deleted");
          setShowDeleteDialog({ open: false, slotId: null });
        }
      }}>
        Yes, Delete
      </Button>

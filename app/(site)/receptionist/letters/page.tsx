...
export default function AdminViewLettersPage() {  const [selectedRole, setSelectedRole] = useState<string | undefined>(undefined);
  const [startDate, setStartDate] = useState<string | undefined>(undefined);
  const [endDate, setEndDate] = useState<string | undefined>(undefined);
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 20;  const roleFilter: 
    | "user" 
    | "admin" 
    | "mda" 
    | "staff" 
    ...
    | "president" 
    | "vice_president" 
    | undefined = selectedRole !== "all" ? (selectedRole as any) : undefined;  const startTimestamp = startDate ? new Date(startDate).getTime() : undefined;
  const endTimestamp = endDate ? new Date(endDate).getTime() : undefined;  const letters = useQuery(api.letters.getAllSubmittedLetters, {
    role: roleFilter,
    ...
    const fetchFileUrls = async () => {
      ...
      for (const letter of letters) {
        if (letter.letterUploadId) {
          try {
            const url = await getFileUrl({ storageId: letter.letterUploadId as Id<"_storage"> });
  ...
  }, [letters, getFileUrl]);  const totalPages = Math.ceil(letters.length / recordsPerPage);
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
     ...
     {/* ✅ Filters Container - Ensures Proper Alignment */}

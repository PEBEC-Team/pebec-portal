...
export default function FillReportPage() {
  ...
  type Role = typeof validRoles[number];  const [template, setTemplate] = useState<null | {
  ...
  const [formData, setFormData] = useState<string[][]>([]);  const convexUser = useQuery(api.users.getUserByClerkId, user?.id ? { clerkUserId: user.id } : "skip");
  const convexUserId = convexUser?._id;  const availableReports = useQuery(
    api.internal_reports.getAvailableReports,
    convexUser?.role && validRoles.includes(convexUser.role as Role)
    ...
    if (fillId && availableReports.length > 0) {
      ...
      if (foundTemplate) {
        ...
        setFormData([foundTemplate.headers.map(() => "")]);      } else {
        toast.error("Report template not found.");
        router.push("/magistrates/reports");      }
    }
  }, [fillId, availableReports, router]);  const submitReport = useMutation(api.internal_reports.submitInternalReport);  const handleChange = (rowIndex: number, colIndex: number, value: string) => {
    setFormData((prev) => {
      const updated = [...prev];
              ...
              <TableRow key={rowIndex}>
                {row.map((cell, colIndex) => (
                 ...
                 {template.headers[colIndex].type === "dropdown" ? (
                   ...
                   <input
                     type="checkbox"
                     checked={cell === "true"}                     onChange={(e) => handleChange(rowIndex, colIndex, e.target.checked ? "true" : "false")}

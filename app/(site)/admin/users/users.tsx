...
export default function Admin() {
    ...
    try {
      ...
      if (["reform_champion", "state_governor", "saber_agent", "magistrates", "deputies"].includes(selectedRole)) {
        if (!selectedState || selectedState.trim() === "") {
          toast("Missing State", { description: "Please select a state for this role." });
          setIsLoading(false);
          return;
        }
      ...
      const convexPayload = {
        ...
        role: selectedRole,
        ...(selectedRole === "staff" && selectedStream ? { staffStream: selectedStream } : {}),
        ...(selectedRole === "admin" && permissions.length > 0 ? { permissions } : {}),
      ...
      if (!["mda", "reform_champion"].includes(selectedRole) && wasInMDA) {
        await removeUserFromMDA({ clerkUserId: selectedUser });
      }
  ...
  .filter((user) => {
    const matchesSearch = [user.firstName, user.lastName, user.email, user.phoneNumber]
      .some((field) => field?.toLowerCase().includes(searchTerm.toLowerCase()));

    const matchesRole = selectedRoleFilter === "all" || user.role === selectedRoleFilter;
    const matchesStream =
      selectedStreamFilter === "all" ||
      (user.role === "staff" && user.staffStream === selectedStreamFilter);
...
<div className="bg-white border border-gray-200 rounded-lg p-4 md:p-6 shadow-sm mb-6">
  <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
    ...
    {/* 🧭 Filter Staff by Stream (shown only if staff is selected) */}
...
<Table className="min-w-[1200px]">
  ...
  : user.mdaName ?? "—"}
          ...
          <Dialog
  open={activeUserId === user.clerkUserId}
  onOpenChange={(open) => {
    if (!open) {
      setSelectedUser(null);
      setActiveUserId(null);
  ...
  <div className="mt-4">
    ...
    <div className="flex flex-col gap-2 text-sm">
      {[
        ...
        { label: "ReportGov - Tickets", value: "/admin/tickets" },
        ...
        <label key={item.value} className="flex items-center gap-2">
          ...
          {item.label}
                ...
                {/* State Selector */}
                {[ "state_governor", "saber_agent", "magistrates", "deputies"].includes(selectedRole) && (
                  <Select
                    value={selectedState}

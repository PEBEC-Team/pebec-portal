...
export default function SaberMaterialsPage() {
  ...
  const renderMaterials = (materials: any[]) => {
    const paginated = materials.slice(
      ...
      <>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
          {paginated.map((mat) => (
            <div
              key={mat._id}
              className="bg-white rounded-xl border shadow-sm hover:shadow-md transition p-4 flex flex-col justify-between h-full"
            >
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        {references.map((folder) => (
          <div
            key={folder}
            onClick={() => setOpenFolder(folder === openFolder ? null : folder)}
            className={`relative group flex flex-col items-center justify-center cursor-pointer transition-transform hover:scale-105 ${openFolder === folder ? 'opacity-100' : 'opacity-60'}`}
          >
      ...
      {openFolder ? renderMaterials(groupedMaterials[openFolder]) : (
        <p className="text-sm text-muted-foreground text-center">Select a folder to view materials.</p>
      )}

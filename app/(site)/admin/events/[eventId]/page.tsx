...
export default function EventDetailsPage() {
  ...
  const event = useQuery(api.events.getEventDetails, { eventId });
  ...
  const filteredRegistrations = useMemo(() => {
    ...
    return registrations.filter((reg) => {
      ...
      return (
        reg.ticketNumber?.toLowerCase().includes(term) ||
  ...
  const handleExport = (format: 'pdf' | 'excel', filter?: 'vip' | 'general') => {
    ...
    ? filteredRegistrations.filter((reg) => (filter === 'vip' ? reg.isVip : !reg.isVip))
  ...
  <Button
    onClick={() => toggleSignUps({ eventId, disable: !event.signUpsDisabled })}

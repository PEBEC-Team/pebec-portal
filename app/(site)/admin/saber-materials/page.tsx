...
export default function SaberMaterialsPage() {
  ...
  const filteredMaterials =
    selectedRole === "all"
      ? allMaterials
      : allMaterials?.filter((m) => m.roles.includes(selectedRole));
      ...
      {filteredMaterials ? (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredMaterials.map((mat) => (
            <div
              key={mat._id}
              className="card w-full min-h-[280px] bg-[#07182E] rounded-2xl overflow-hidden relative transition-all duration-300 hover:shadow-[0_0_30px_rgba(0,183,255,0.5)] flex flex-col"
            >
              <div className="card-content p-4 relative z-10 flex flex-col justify-between h-full">
                <div>
                  <div className="flex items-start mb-4 gap-3">
                    ...
                    <div className="flex-1">
                      <h2
                        className="text-lg font-bold text-white/90 break-words leading-snug line-clamp-2"
                        title={mat.title}
                      >
                        {mat.title}
                      </h2>
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-xs text-white/60 line-clamp-3">{mat.description}</p>
                  </div>
                </div>

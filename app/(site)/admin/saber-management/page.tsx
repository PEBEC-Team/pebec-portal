...
export default function SaberManagementPage() {
  ...
  useEffect(() => {
    const fetchFileUrls = async () => {
      ...
      for (const mat of materials) {
        if (mat.fileId) {
          try {
            const url = await getStorageUrl({ storageId: mat.fileId });
        ...
        <TabsContent value="dli">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...dlis].sort((a, b) => a.number - b.number).map((dli) => (
              <Card key={dli._id} className="p-4 flex flex-col justify-between">
                <CardContent>
  ...
  ✏️ Edit
                  ...
                  <Button size="sm" variant="destructive" onClick={() => deleteDLI({ id: dli._id })}>
        ...
        <TabsContent value="berap">
          ...
          <ul className="space-y-3">
            {beraps.map((b) => (
              <li key={b._id} className="border p-4 rounded bg-white flex justify-between items-start">
                <div>
                  <div className="text-lg font-semibold">{b.year} - {b.title}</div>
                  <p className="text-sm text-muted-foreground">{b.description}</p>
                </div>
                <Button size="sm" variant="destructive" onClick={() => deleteBERAP({ id: b._id })}>
                  <Trash className="w-4 h-4 mr-1" /> Delete
                </Button>
              </li>
            ))}
...
<TabsContent value="materials">
  ...
  <ul className="space-y-3">
    {materials.map((mat) => {
      const parent =
        mat.parentType === "dli"
          ? dlis.find((d) => d._id === mat.parentId)
          : beraps.find((b) => b._id === mat.parentId);

      const parentLabel =
        mat.parentType === "dli" && parent
          ? `DLI ${(parent as any).number}: ${(parent as any).title}`
          : mat.parentType === "berap" && parent
          ? `${(parent as any).year} - ${(parent as any).title}`
          : "Unknown";

      return (
        <li key={mat._id} className="border p-4 rounded bg-white flex justify-between items-center">
          <div>
          ...
          <div className="flex gap-2">
            {(mat.link || fileUrls[mat._id]) && (
              <a
                href={mat.link || fileUrls[mat._id]}
                target="_blank"
            ...
            <Button
              size="sm"
              variant="destructive"
              onClick={() => deleteMaterial({ id: mat._id })}
            >
              <Trash className="w-4 h-4 mr-1" /> Delete
            </Button>
          </div>
...
/>
      ...
      <AddMaterialModal
        ...
        dliOptions={dlis.map((d) => ({ id: d._id as Id<"dli">, label: `DLI ${d.number}` }))}
        berapOptions={beraps.map((b) => ({ id: b._id as Id<"berap">, label: `${b.year} - ${b.title}` }))}
        onSave={async (data) => {
          await uploadMaterial({ ...data, uploadedAt: Date.now() });

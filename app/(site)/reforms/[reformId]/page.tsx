...
import { Accordion, AccordionItem } from "@/components/Accordion";interface Reform {
  _id: Id<"reforms">;
  title: string;
  description: string;
  category: string;
...
export default function ReformDetailPage() {
  const { reformId } = useParams();
  const reform = useQuery(api.reforms.getReformById, { id: reformId as Id<"reforms"> }) || null;
  const imageUrl = useQuery(api.reforms.getImageUrl, reform?.imageId ? { storageId: reform.imageId } : "skip");  const [open, setOpen] = useState(false);
  ...
  return (
    <div className="container mx-auto p-6 mt-30">
      <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden rounded-lg">
        {imageUrl && (
          <Image
            ...
            layout="fill"
            objectFit="cover"
            className="rounded-lg blur-sm brightness-50"          />
        )}
              ...
              <DialogContent className="max-w-2xl">
                ...
                <div className="w-full">
                  <iframe
                    ...
                    src={reform.videoLink.replace("watch?v=", "embed/")}                    allowFullScreen
                  ></iframe>
                </div>
              </DialogContent>
            </Dialog>
      ...
      {reform.impact.length > 0 && (
        <div className="mt-10 bg-white p-8 rounded-lg shadow-lg">
          ...
          <Accordion>
            {reform.impact.map((impact, index) => {
              const Icon = impactIcons[index % impactIcons.length];              return <AccordionItem key={index} title={`Impact #${index + 1}`} fullText={impact} icon={<Icon />} />;
            })}
          </Accordion>
        </div>
        )}
        ...
        {reform.outcome.length > 0 && (
        <div className="mt-12 bg-green-900 text-white p-12 rounded-lg shadow-lg">
          <h2 className="text-3xl font-semibold text-center">This reform has generated the following outcomes</h2>
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {reform.outcome.map((item, index) => (
              <div key={index} className="flex items-start gap-4 bg-green-800 p-6 rounded-lg shadow-md transition hover:bg-green-700">
                {/* ✅ Fix: Set Icon Size to be Fixed */}

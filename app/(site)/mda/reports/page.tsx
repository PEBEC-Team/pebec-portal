...
export default function ReportsPage() {
  ...
  const [fileUrls, setFileUrls] = useState<Record<string, string>>({});  const [isReportGeneratorOpen, setIsReportGeneratorOpen] = useState(false);  const convexUser = useQuery(api.users.getUserByClerkId, user?.id ? { clerkUserId: user.id } : "skip");
  const convexUserId = convexUser?._id;  const availableReports = useQuery(
        ...
        try {
          const urls = await Promise.all(
            submittedReports.map(async (report) => {
              if (report.fileId) {
                const url = await getFileUrl({ storageId: report.fileId });                return { fileId: report.fileId, url: url ?? "" };
              }
              return null;
            })
          );
          ...
          const urlMap = urls
            .filter((entry) => entry !== null)            .reduce(
              (acc, entry) => {
                if (entry) acc[entry.fileId] = entry.url ?? "";                return acc;
              },
  ...
  .sort((a, b) => b.submittedAt - a.submittedAt);  const filteredReports = submittedReportsWithTitles.filter((report) => {
  ...
  const exportToPDF = (report) => {
    ...
    doc.setFontSize(12);
    doc.text(`Submitted On: ${new Date(report.submittedAt).toLocaleDateString()}`, 15, 25);    const columnStyles: UserOptions["columnStyles"] = report.headers.reduce(
      (styles, _, index) => ({
        ...styles,
    ...
    autoTable(doc, {
      startY: 35,
      head: [report.headers],      body: report.data,      theme: "striped",
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([report.headers, ...report.data]);    const workbook = XLSX.utils.book_new();
  ...
  return (
    <div className="p-4 md:p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mt-6 gap-4">
        ...
        <div className="flex flex-col mb-5 sm:flex-row flex-wrap gap-4 mt-6">
        <Button className="bg-green-600 text-white w-full sm:w-auto" onClick={() => setIsUploadModalOpen(true)}>
          📤 Upload Report
        </Button>

...
export default function ReportsPage() {
  ...
  const [isReportGeneratorOpen, setIsReportGeneratorOpen] = useState(false);  const convexUser = useQuery(api.users.getUserByClerkId, user?.id ? { clerkUserId: user.id } : "skip");
  ...
  onClick={async () => {
    ...
    try {
      ...
      link.href = blobUrl;
      link.download = fileMeta.fileName || "downloaded_file";      document.body.appendChild(link);
      link.click();
...
<Button
  ...
  onClick={() => exportToPDF(report, convexUser)}>

...
export default function ReceivedLettersPage() {
  ...
  const filtered = allLetters.filter((l) => {
    ...
    const letterStatus = l.status ?? "sent";    const matchStatus = filters.status ? letterStatus === filters.status : true;
  ...
  const paginated = sorted.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);  useEffect(() => {
    const fetchUrls = async () => {
      const newUrls: { [key: string]: { url: string; fileName: string } } = {};
  
      for (const letter of paginated) {
        if (letter.letterUploadId && !fileUrls[letter._id]) {
          const fileData = await getFileUrl({ storageId: letter.letterUploadId });
          if (fileData) {
  ...
  onChange={(e) => {
    ...
    setFilters((prev) => ({
      ...prev,
      dateFrom: newFrom,
      dateTo: prev.dateTo && newFrom > prev.dateTo ? "" : prev.dateTo,
    }));
  }}
/>
<Input

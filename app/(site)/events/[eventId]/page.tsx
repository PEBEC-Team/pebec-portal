...
import html2canvas from "html2canvas";import ReactDOMServer from "react-dom/server";

export default function EventPage() {
  const { eventId } = useParams();
  const [ticketData, setTicketData] = useState<any>(null);
  const [isTicketReady, setIsTicketReady] = useState(false);
  const [answers, setAnswers] = useState<{ questionId: Id<"event_questions">; answer: string }[]>([]);
...
const [vipCode, setVipCode] = useState("");
  ...
  const [isClient, setIsClient] = useState(false);  const event = useQuery(api.events.getEventById, isClient ? { eventId: eventId as Id<"events"> } : "skip");
  const questions = useQuery(api.events.getEventQuestions, isClient ? { eventId: eventId as Id<"events"> } : "skip");
  ...
  const coverImageUrlQuery = useQuery(api.images.getImageUrl, storageId ? { storageId } : "skip");
  ...
  const [ticketsLeft, setTicketsLeft] = useState<{ vip: number | null; general: number | null }>({ vip: null, general: null });
  const registrations = useQuery(api.events.getEventRegistrations, isClient ? { eventId: eventId as Id<"events"> } : "skip");
  ...
  const generateStyledPdfBlobFromTicket = async (ticketObject) => {
    ...
    const pdf = new jsPDF({
      ...
      unit: "mm",
      format: [210, 80],    });    pdf.setFillColor(255, 255, 255);    pdf.roundedRect(5, 5, 200, 70, 15, 15, "F");    pdf.setFillColor(235, 235, 235);
    pdf.roundedRect(10, 10, 55, 60, 10, 10, "F");    pdf.setFillColor(255, 255, 255);
    pdf.roundedRect(70, 10, 70, 60, 10, 10, "F");    pdf.setFillColor(0, 85, 50);
    pdf.roundedRect(145, 10, 55, 60, 10, 10, "F");    pdf.setDrawColor(150);
    pdf.setLineDashPattern([2, 2], 0);
    pdf.line(67, 10, 67, 70);    const logoPath = "/images/logo/logo_pebec1.PNG";
    pdf.addImage(logoPath, "PNG", 88, 12, 30, 12);    pdf.setFontSize(14);
    pdf.setTextColor(30, 30, 30);
    pdf.setFont("helvetica", "bold");
    const eventTitleLines = pdf.splitTextToSize(ticketObject.event.title.toUpperCase(), 65);
    pdf.text(eventTitleLines, 105, 35, { align: "center" });    pdf.setFontSize(10);
    pdf.setTextColor(100, 100, 100);
    const ticketNumberLines = pdf.splitTextToSize(`Ticket #: ${ticketObject.ticketNumber}`, 65);
    pdf.text(ticketNumberLines, 105, 65, { align: "center" });    pdf.setFont("helvetica", "bold");
pdf.setFontSize(10);
pdf.text(`Access: ${ticketObject.isVip ? "VIP" : "General"}`, 105, 55, { align: "center" });
const formattedType = ticketObject.eventType === "vip_and_general" ? "VIP & General" : ticketObject.eventType;
pdf.text(`Event Type: ${formattedType}`, 105, 60, { align: "center" });    if (ticketObject.qrCodeUrl) {
      pdf.addImage(ticketObject.qrCodeUrl, "PNG", 20, 20, 35, 35);
    }    const eventDate = new Date();
    const barcodeDate = eventDate
      .toLocaleString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: false })
      .replace(/[\/,: ]/g, "");    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(8);
    ...
    pdf.setFontSize(12);
    pdf.setTextColor(30, 30, 30);
    pdf.text(eventDate.toLocaleString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }), 15, 63);    pdf.setFontSize(12);
    ...
    pdf.text(locationLines, 150, 27);    pdf.setFontSize(12);
    pdf.setFont("helvetica", "bold");
    pdf.text("Date & Time:", 150, 40);
    pdf.setFontSize(10);
    pdf.setFont("helvetica", "normal");
    const dateTimeLines = pdf.splitTextToSize(ticketObject.eventDate, 50);
    pdf.text(dateTimeLines, 150, 47);    pdf.setFontSize(12);
    pdf.setFont("helvetica", "bold");
...
if (!currentUser) {  const existing = await convex.query(api.events.getRegistrationByEmail, {
  ...
  if (existing) {
    ...
    try {
      ...
      const ticketNumber = `PEBEC-EV-${day}${month}${year}-${index}`;      const qrCodeUrl = await QRCode.toDataURL(ticketNumber);      const ticketOwner = currentUser
      ...
      const ticketObject = {
        ticketNumber,
        event: { ...event, coverImageUrl: coverImageUrl || "" },
        qrCodeUrl,
      ...
      setTimeout(() => URL.revokeObjectURL(pdfUrl), 5000);      const uploadUrl = await generateUploadUrl();
      ...
      const { storageId } = await response.json();      await rsvpEventMutation({
        eventId: eventId as Id<"events">,
        answers,
...
const generatePdfFromTicket = async () => {
  ...
  ticketElement.style.width = "fit-content";  ticketElement.style.height = "auto";
  ticketElement.style.minHeight = "fit-content";  ticketElement.style.padding = "20px";
  ...
  } finally {    ticketElement.style.display = "none";
    setTicketData(null);  }
};
...
const isFormIncomplete = Boolean(
  ...
  !email.trim() ||
  questions?.some(
    (q) => !answers.find((a) => a.questionId === q._id)?.answer.trim()
  ) ||
  (
    (event?.eventType === "vip" || (event?.eventType === "vip_and_general" && isVip)) &&
    event?.vipAccessCode &&
    vipCode.trim() !== event.vipAccessCode.trim()
  )
);
  ...
  return (
    <div className="relative mx-auto w-full bg-white pt-12 mt-30 px-10 md:px-30 lg:px-30 md:mb-20  ">
      <div className="flex flex-col md:flex-row gap-8">


        {/* RSVP Form Section */}
        <div className="md:w-1/2 w-full">
  {event?.signUpsDisabled ? (
    ...
    <>
        ...
        <div>
          ...
          <input
            ...
            className={`mt-1 block w-full border p-2 rounded-md ${
              vipCodeError ? "border-red-500" : "border-gray-300"
            }`}
            placeholder="Enter VIP code"
          />
          {vipCodeError && <p className="text-sm text-red-500 mt-1">{vipCodeError}</p>}
          {vipCodeValid && (
            <p className="text-sm text-green-600 mt-1">✅ The code you entered is correct</p>
          )}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col space-y-4">
        ...
        <Button
          type="submit"
          disabled={loading || isPastEvent || isFormIncomplete}
          className={`w-full text-white ${
            isPastEvent || isFormIncomplete
  ...
  <div className="bg-green-800 text-white p-6 rounded-xl shadow-lg space-y-5">
    ...
    {ticketData && (
        <div className="flex flex-col items-center mb-10">
          ...
          {/* Download Button */}
          <Button
            onClick={generatePdfFromTicket}
            className="mt-4 px-6 py-3 bg-green-600 text-white rounded-lg shadow-lg hover:bg-blue-700 transition"
          >
            Download Your Ticket
          </Button>
          {/* Hidden Desktop Ticket */}

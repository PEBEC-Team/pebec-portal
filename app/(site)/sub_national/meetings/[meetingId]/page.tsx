...
export default function MeetingDetails() {
  ...
  const meeting = useQuery(api.meetings.getMeetingById, meetingId ? { meetingId } : "skip");
  ...
  const attendees = useQuery(api.meetings.getAttendees, meetingId ? { meetingId } : "skip");
  ...
  const handleCancelMeeting = async () => {
    ...
    try {
      await cancelMeeting({ meetingId, userId: convexUserId });
  ...
  return (
    <div className="flex items-center justify-center bg-gray-50 px-4 py-10">
      <div className="w-full max-w-2xl bg-white shadow-md rounded-lg p-6">
        ...
        <div className="mt-5 border-t pt-4 space-y-2">
          ...
          <p className="text-sm flex items-center gap-2">
            <CircleCheck className={`size-18 ${
              meeting.status === "confirmed" ? "text-green-500" 
          ...
          <div className="mt-6 border-t pt-4">
            ...
            <div className="mt-3 space-y-2">
              {attendees.map((attendee) => (
                <div key={attendee._id} className="flex items-center gap-3 bg-gray-100 rounded-lg px-3 py-2">
                  <Image 
                    src={attendee.imageUrl || "/default-avatar.png"} 
                    alt="User"
                    width={40} 
                    height={40} 
                    className="rounded-full border"
                  ...
                  <div>
                    ...
                    <p className={`text-xs ${
                      meeting.acceptedAttendees.includes(attendee._id) 
                      ? "text-green-500" 
                      : "text-yellow-500"
                    }`}>
        ...
        {/* Action Buttons */}
        {isCreator && meeting.meetingDate > Date.now() && (

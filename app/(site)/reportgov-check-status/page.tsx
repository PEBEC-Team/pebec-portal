...
export default function CheckStatusPage() {
  ...
  const ticket = useQuery(
    api.tickets.getPublicTicketByReference,
    submitted && isValid
      ? {
          ticketNumber,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 py-16 px-6 mt-25">
      <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-3xl overflow-hidden grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="p-10 md:p-14 flex flex-col justify-center">
          ...
          <div className="space-y-4">
            <Input
              placeholder="Ticket Number (e.g. REP-23456)"
              value={ticketNumber}
          ...
          {loading && (
            <p className="mt-4 text-sm text-gray-500 animate-pulse text-center md:text-left">
              Searching for your complaint...
            </p>
          )}

          {submitted && !loading && ticket === null && (
            <p className="mt-4 text-sm text-red-500 font-medium text-center md:text-left">
              No ticket found with those details.
            </p>
          )}

          {ticket && (
            <div className="mt-6 bg-gray-50 p-5 rounded-lg border text-sm text-gray-700 space-y-1">
              ...
              <Link
                href={`/reportgov-guest/tickets/${ticket._id}`}
        ...
        <div className="hidden md:flex items-center justify-center bg-blue-100">
          <img
            src="/images/check_status.svg"
            alt="Status Illustration"
            className="max-w-md object-contain p-10"
          />
        </div>
      </div>
    </div>

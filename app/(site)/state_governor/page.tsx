...
export default function StateDLIsPage() {
  ...
  const state = userData?.state || "your state";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      {showWelcome && (
        <div className="bg-green-100 border border-green-300 text-green-800 px-6 py-4 rounded-lg mb-6 relative shadow-md flex items-start justify-between">
          <div>
      ...
      <div className="text-center mb-6">
        {userData.state && (
          <span className="bg-green-100 text-black-800 text-sm font-semibold px-4 py-1 rounded-full shadow inline-block mb-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {stateDLIs.map((dli) => {
          ...
          return (
            <div key={dli._id} className="bg-white shadow-xl rounded-2xl p-6 flex flex-col h-full relative">
              {/* Header */}
              ...
              <div className="mt-6">
                <Button
                  onClick={() => router.push(`/state_governor/dli/${dli._id}`)}

...
export default function MediaDetailPage() {
  ...
  const media = useQuery(api.media.getMediaById, mediaId ? { mediaId } : "skip");
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 mt-30 relative">
      {/* Back Button */}
      <button
        onClick={() => router.push("/media")}
        className="absolute top-2 left-2 px-4 py-2 text-sm rounded-md bg-gray-100 hover:bg-gray-200 transition"
      >
        ← Back to Media
      </button>
      ...
      <div className="flex justify-between items-center mb-4 mt-8">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 border border-blue-500 text-blue-600 rounded-md hover:bg-blue-50"
          >
            Share <FaShareAlt size={16} />
          </button>

          {shareOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-md z-50 overflow-hidden">
              <a
...
<Image
  src={media.pictureUrls[currentImage] ?? "/fallback.jpg"}
  alt={`Media image ${currentImage + 1}`}
  fill
  className="object-contain rounded-lg transition-all duration-300"
  quality={30}
  placeholder="blur"
  blurDataURL="/blur_placeholder.png"
/>
  ...
  <button
    onClick={handlePrev}
    className="absolute top-1/2 -translate-y-1/2 left-2 z-10 bg-white/70 p-2 rounded-full"
  >
    ←
  </button>
  <button
    onClick={handleNext}
    className="absolute top-1/2 -translate-y-1/2 right-2 z-10 bg-white/70 p-2 rounded-full"
  >
    →
  </button>
</div>
...
<div className="flex gap-2 overflow-x-auto justify-center md:justify-start px-1">
  {media.pictureUrls.map((url, idx) => (
    <div
    key={idx}
    className={`relative w-20 h-20 md:w-24 md:h-24 rounded-md cursor-pointer border-2 ${
  ...
  >
    ...
    />
      ...
      {/* Videos */}
      {Array.isArray(media.videoUrls) && media.videoUrls.length > 0 && (
        <div className="mb-6">
          ...
          <div className="flex flex-col gap-6">
            {media.videoUrls.map((url, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="aspect-video w-full bg-black rounded overflow-hidden"
                >
                  {embed ? (
                    <iframe
                      src={embed}
                    ...
                    <div className="text-white p-6">Invalid video link</div>
                  )}
                </div>
              );
            })}
